...
class SENE_MySQLi_Engine
{
    ...
    public function __construct()
    {
        ...
        $this->__mysqli->set_charset('utf8mb4'); // by Muhammad Sofi 23 December 2021 14:00 | changing charset
    ...
    public function query($sql, $cache_enabled=0, $flushcache=0, $type="object")
    {
        ...
        if ($cache_enabled) {
            ...
            if (is_file(SENECACHE.'/'.$cache) && file_exists(SENECACHE.'/'.$cache)) {
                //die("wololo");
                $dataz=array();
                $fp = fopen(SENECACHE.'/'.$cache, "r");
                $sz = filesize(SENECACHE.'/'.$cache);
                if ($sz>0) {
                    $str = fread($fp, $sz);
        ...
        if (is_array($params)) {
            ...
            switch ($comp) {
                ...
                default:
                    ...
                    if ($v=="IS NULL" || $v=="is null" || $v=="IS NOT NULL" || $v=="is not null") {
                        $v = strtoupper($v);
                        $c = "";
                        $val = $v;
                    } else {
