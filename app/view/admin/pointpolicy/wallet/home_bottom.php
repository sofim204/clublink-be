...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/game/ticket_shop/"); ?>",
	...
	$('.dataTables_filter input').attr('placeholder', 'Search earned ticket').css({'width':'250px', 'display':'inline-block'});
}
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/game/ticket_shop/hapus/'); ?>'+ieid;
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?=base_url_admin('game/ticket_shop/edit/');?>'+ieid;
	},333);
});
...
$("#ficon_change").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	var url = '<?=base_url("api_admin/game/ticket_shop/change_icon/")?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
