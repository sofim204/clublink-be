...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"sAjaxSource"		: "<?=base_url("api_admin/redemptionexchange/notification/")?>",
			...
			"fnServerData"		: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						var url = '<?=base_url()?>api_admin/redemptionexchange/notification/detail/'+id;
						NProgress.start();
...
<!-- SUCCESS -->
$("#b_success").on("click", (e) => {
    e.preventDefault();
	$("#modal_success_option").modal("show");
...
$("#b_success_option_1").on("click", (e) => {
    e.preventDefault();
    let confirm_message = confirm("Are you sure to approve this data with this reason?"); // same like takedown
    if(confirm_message) {
        NProgress.start();
		var reason = 1;
        var url = '<?=base_url() ?>api_admin/redemptionexchange/notification/success/?id=' +ieid +'&reason=' +reason;
        $.get(url).done(function(response){
            NProgress.done();
            if(response.status=="200"){
                ...
                setTimeout(function(){
					...
					$("#modal_notification").modal("hide");
					$("#modal_success_option").modal("hide");
                }, 400);
            }else{
...
$("#b_problem").on("click", (e) => {
    ...
    if(confirm_message) {
        NProgress.start();
        var url = '<?=base_url() ?>api_admin/redemptionexchange/notification/problem/?id=' +ieid;
        $.get(url).done(function(response){
            NProgress.done();
            if(response.status=="200"){
...
$("#b_refund").on("click", (e) => {
    ...
    if(confirm_message) {
        NProgress.start();
        var url = '<?=base_url() ?>api_admin/redemptionexchange/notification/refund/?id=' +ieid;
        $.get(url).done(function(response){
            NProgress.done();
            if(response.status=="200"){
...
$("#bdownload_xls").on("click",function(e){
	...
	if (cdate_start == '' || cdate_start == null) {
		...
		var url = '<?=base_url("api_admin/redemptionexchange/notification/download_xls/?")?>';
