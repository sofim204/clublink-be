...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/summary/buyer/product/1/")?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					gritter('<h4>Error</h4><p>Cannot get buyer product</p>','warning');
					NProgress.done();
				});
			},
	});
...
function getSummary(){
  var url = '<?=base_url("api_admin/summary/buyer/index/".$pelanggan->id)?>';
  NProgress.start();
  $.get(url).done(function(dt){
    NProgress.done();
    if(dt.status == 200){
      $("#buyer-order-count").html(""+dt.data.order_count);
      $("#buyer-sales-total").html("$"+dt.data.sales_total);
      $("#buyer-product-count").html(dt.data.product_count);
      $("#buyer-freeproduct-count").html(dt.data.freeproduct_count);
      $("#buyer-rejected-count").html(""+dt.data.rejected_count);
      $("#buyer-confirmed-count").html(""+dt.data.confirmed_count);
    }
  }).fail(function(){
    NProgress.done();
  })
