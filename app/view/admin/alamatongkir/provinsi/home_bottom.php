...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php  echo base_url("api_admin/alamatongkir/provinsi/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var url = '<?php echo base_url(); ?>api_admin/alamatongkir/provinsi/detail/'+id;
						$.get(url).done(function(response){
							if(response.status==100 || response.status=='100'){
								var dta = response.result;
								ieid = dta.id;
								$("#ieid").val(dta.id);
								$("#ienegara_id").val(dta.negara_id);
								$("#ienama_provinsi").val(dta.nama_provinsi);
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?php echo base_url("api_admin/alamatongkir/provinsi/tambah/"); ?>';
	$.ajax({
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/alamatongkir/provinsi/edit/"); ?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?php echo base_url('api_admin/alamatongkir/provinsi/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status=="100" || response.status==100){
