...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php  echo base_url("api_admin/alamatongkir/jne/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var url = '<?php echo base_url(); ?>api_admin/alamatongkir/jne/detail/'+id;
						$.get(url).done(function(response){
							if(response.status==100 || response.status=='100'){
								var dta = response.result;
								ieid = dta.id;
								$("#ieid").val(dta.id);
								$("#ieorigin").html('<option value="'+dta.origin+'">'+dta.origin+'</option>');
								$("#ieorigin").val(dta.origin);
                $("#iekode_jne").val(dta.kode_jne);
								$("#iekabkota").val(dta.kabkota);
                $("#iekecamatan").val(dta.kecamatan);
                $("#ieoke15_tarif").val(dta.oke15_tarif);
                $("#ieoke15_est").val(dta.oke15_est);
                $("#iereg15_tarif").val(dta.reg15_tarif);
                $("#iereg15_est").val(dta.reg15_est);
                $("#ieyes15_tarif").val(dta.yes15_tarif);
                $("#ieyes15_est").val(dta.yes15_est);
...
$("#modal_tambah").on("shown.bs.modal",function(e){
	...
	//listener
	$("#ipropinsi").off("change");
	$("#ipropinsi").on("change",function(e){
		e.preventDefault();
...
function getProvinsi(nama_provinsi=""){
	...
	$("#ipropinsi").empty();
	$("#iepropinsi").empty();
	$("#ipropinsi").html('<option value="">Loading...</option>');
	$.get(url).done(function(hasil){
		...
		if(hasil.status == 100 || hasil.status == "100"){
			...
			$("#ipropinsi").html(isi);
			$("#iepropinsi").html(isi2);
			if(selected != '') $("#iepropinsi").val(selected);
...
$("#ftambah").on("submit",function(e){
	...
	if(fd.has('propinsi')){
		fd.delete('propinsi');
		fd.append('propinsi',$("#ipropinsi option:selected").text());
	}
	if(fd.has('kabkota')){
		fd.delete('kabkota');
	...
	var url = '<?php echo base_url("api_admin/alamatongkir/jne/tambah/"); ?>';
	$.ajax({
...
$("#modal_edit").on("shown.bs.modal",function(e){
	//
	//listener
	$("#iepropinsi").off("change");
	$("#iepropinsi").on("change",function(e){
		e.preventDefault();
		var provinsi_id = $(this).val();
		getKabkota(provinsi_id);
	});
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/alamatongkir/jne/edit/"); ?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?php echo base_url('api_admin/alamatongkir/jne/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status=="100" || response.status==100){
					growlType = 'success';
