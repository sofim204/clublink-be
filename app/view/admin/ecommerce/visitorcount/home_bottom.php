...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//showModalPleaseWait();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/ecommerce/visitorcount/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
				    ...
				    $.ajax({
						url: '<?=base_url("api_admin/ecommerce/visitorcount/totalvisitbydevice")?>',
						data:{
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var total_signup_login = $('#drTable').DataTable().row(currentRow).data()[4];
						var url_data = '<?=base_url() ?>api_admin/ecommerce/visitorcount/getData/' + mob_type + '/' + cdate;
						...
						$.get(url_data).done(function(response){
							if(response.status==200){
								...
								$("#detail_total").val(total_signup_login);
...
if(jQuery('#drTableDetailLog').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		"sAjaxSource"		: "<?=base_url("api_admin/ecommerce/visitorcount/detail_data_log"); ?>",
		"fnServerParams": function ( aoData ) {
			aoData.push(
		...
		"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
			oSettings.jqXHR = $.ajax({
				...
				//var url_total = '<?=base_url() ?>api_admin/ecommerce/visitorcount/getTotal/' + mobile_type_history + '/' + detail_date_history;
...
$("#bdownload_xls").on("click", function(e) {
	...
	//} else {
		var url ='<?=base_url_admin()?>ecommerce/visitorcount/download_xls/?';
...
$("#btn_udid_account").on("click", function(e) {
	e.preventDefault();
	window.location = '<?=base_url_admin('ecommerce/udid_account/');?>';
});