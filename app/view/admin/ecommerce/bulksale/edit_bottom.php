...
$("#fedit").on("submit",function(e){
	...
	$.ajax({
		url: '<?=base_url('api_admin/ecommerce/bulksale/edit/'.$produk->id); ?>',
		type: "POST",
		data: fd,
		contentType: false,
		cache: false,
		...
		success: function(data){
			...
			if(data.status == 200){
				...
				setTimeout(function(){
					window.location = '<?=base_url_admin('ecommerce/bulksale/'); ?>';
				},3000);
			}else{
...
function getProductImage(){
	...
	var url_produk = '<?=base_url()?>api_admin/ecommerce/bulksale/image/<?=$produk->id?>';
	$.get(url_produk).done(function(dt){
		...
		if(dt.status == 200 || dt.status == '200'){
			...
			$("#dimage_list").on("click",'.btn-cover-set',function(e){
				...
				if(c){
					NProgress.start();
					$.get('<?=base_url("api_admin/ecommerce/bulksale/gambar_cover/".$produk->id.'/')?>'+encodeURIComponent(dtid)).done(function(res){
						NProgress.done();
						if(res.status == 200){
			...
			$("#dimage_list").on("click",'.btn-remove-gambar',function(e){
				...
				if(c){
					NProgress.start();
					$.get('<?=base_url("api_admin/ecommerce/bulksale/gambar_hapus/".$produk->id)?>/'+encodeURIComponent(cpfid)).done(function(res){
						NProgress.done();
						if(res.status == 200){
...
$("#image_upload_modal_form").on("submit",function(e){
	...
	NProgress.start();
	$.ajax({
		url: '<?=base_url('api_admin/ecommerce/bulksale/gambar_upload/'.$produk->id); ?>', // Url to which the request is send
		type: "POST",
