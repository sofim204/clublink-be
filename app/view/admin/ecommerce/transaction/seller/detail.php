<?php
	...
	if (!isset($order->shipping->nama)) {
	$order->shipping->nama = '';
	}
	if (!isset($order->shipping->telp)) {
	$order->shipping->telp = '';
	}

	// by Donny Dennison - 3 November 2021 10:00
	// remark code
	// if (!isset($order->shipping->alamat)) {
	//   $order->shipping->alamat = '';
	// }

	if (!isset($order->shipping->alamat2)) {
	$order->shipping->alamat2 = '';
	}
	if (!isset($order->shipping->kecamatan)) {
	$order->shipping->kecamatan = '';
	}
	if (!isset($order->shipping->kabkota)) {
	$order->shipping->kabkota = '';
	}
	if (!isset($order->shipping->provinsi)) {
	$order->shipping->provinsi = '';
	}
	if (!isset($order->shipping->negara)) {
	$order->shipping->negara = '';
	}
	if (!isset($order->shipping->kodepos)) {
	$order->shipping->kodepos = '';
	}
...
<div id="page-content">
	...
	<div class="content-header">
		<div class="row" style="padding: 0.5em 2em;">
			<div class="col-md-12">
				<div class="btn-group">
					<a href="<?=base_url_admin('ecommerce/transaction/seller/'); ?>" class="btn btn-back"><i class="fa fa-chevron-left"></i> Back</a>
				</div>
			</div>
		</div>
	</div>
	<ul class="breadcrumb breadcrumb-top">
		<li>Ecommerce</li>
		<li><a href="<?=base_url_admin('ecommerce/transaction/seller/')?>">Transaction By Seller</a></li>
		<li>Detail</li>
		<li>ORDER ID: #<?=$order->id?></li>
		<li>PRODUCT ID: #<?=$d_order_detail_id?></li>
		...
		<div class="row">
			...
			<table class="table  table-striped  table-borderless">
				...
				<tr>
					...
					$rating_value = (int) $rating->buyer_rating;
				...
				<tr>
					...
					<td>:</td>
					<td> <?php $rating_value = 0; if (isset($rating->buyer_rating)) {
					$rating_value = (int) $rating->seller_rating;
					} echo $this->__toStars($rating_value) ?></td>
				</tr>
				<tr>
					<th>Distance</th>
					<td>:</td>
					<td><?=ceil($d->shipment_distance/1000)?>KM</td>
				</tr>
			</table>
			</div>
			<!--end right-->
		...
		<div class="row">
			<div class="col-md-12">
				<div class="btn-group pull-right">
					<a href="<?=base_url_admin("ecommerce/transactionhistory/detail/$order->id/$d_order_detail_id/")?>" class="btn btn-default btn-alt" target="_blank"><i class="fa fa-external-link"></i> Transaction History</a>
	...
	<div class="block">
		...
		<div class="table-responsive">
			<table class="table table-bordered table-vcenter">
				...
				<tbody>
					<?php $ip = 0; $sub_total = 0.0; foreach ($items as $ods) { ?>
						<?php if ($ods->buyer_status == 'rejected') {
						continue;
						} $sub_total += $ods->qty*$ods->harga_jual; $ip++; ?>
						<tr class="">
							<td class="col-md-1 text-center">
								<img id="produk_image_<?=$ods->c_produk_id?>" src="<?=base_url($ods->foto)?>" class="img-responsive" onerror="this.onerror=null;this.src='<?=base_url()?>media/produk/default.png';" />
							</td>
							<td class="">
								...
								<?php if ($ods->buyer_status == "rejected" && $ods->settlement_status != "paid_to_buyer") { ?>
									<p style="font-size: smaller;margin-bottom: 0;" class=" text-warning"><i class="fa fa-info-circle"></i> Rejected by buyer, waiting for admin action</p>
								<?php } ?>
								<?php if ($ods->buyer_status == "rejected" && $ods->settlement_status == "paid_to_buyer") { ?>
									<p style="font-size: smaller;margin-bottom: 0;" class=" text-warning"><i class="fa fa-info-circle"></i> Rejected by buyer approved</p>
								<?php } ?>
							</td>
							...
							<td class="text-right"><strong><?=$this->__m($ods->harga_jual, 0, ',', '.'); ?></strong></td>
							<td class="text-right"><strong><?=$this->__m(($ods->harga_jual*$ods->qty), 0, ',', '.'); ?></td>
						</tr>
						<?php } ?>
						<?php if (empty($ip)) { ?>
						<tr>
							<td colspan="5" class="text-center text-success ">All item(s) has been rejected by buyer. <a href="<?=base_url_admin("ecommerce/transactionhistory/detail/".$ods->d_order_id."/".$ods->d_order_detail_id)?>#produk_tabel" target="_blank">View history</a></td>
							<td class="text-right"><?=$this->__m(0)?></td>
						</tr>
						<?php } ?>
						<tr class="active">
						...
						<tr class="active">
							...
							<td class="text-right"><strong><?=$this->__m($sub_total+$d->shipment_cost+$d->shipment_cost_add+$d->shipment_cost_sub, 0, ',', '.'); ?></strong></td>
						</tr>
						<tr class="">
		...
		<div class="row">
			...
			<div class="block">
				...
				<table class="table table-striped table-borderless">
					<tr>
						<th>Account Name</th>
						<td>:</td>
						<td><?php if (isset($bank_seller->nama)) {
							echo $bank_seller->nama;
						}?></td>
					</tr>
					<tr>
						<th>Bank Name</th>
						<td>:</td>
						<td><?php if (isset($bank_seller->nama_bank)) {
							echo $bank_seller->nama_bank;
						}?></td>
					</tr>
					<tr>
						<th>Account Number</th>
						<td>:</td>
						<td><?php if (isset($bank_seller->nomor)) {
							echo $bank_seller->nomor;
						}?></td>
					</tr>
				</table>
			</div>
			...
			<div class="block">
				...
				<?php if (!empty($bank_buyer->nama)) { ?>
					...
					<tr>
						<th>Account Name</th>
						<td>:</td>
						<td><?php if (isset($bank_buyer->nama)) {
							echo $bank_buyer->nama;
						} ?></td>
					</tr>
					<tr>
						<th>Bank Name</th>
						<td>:</td>
						<td><?php if (isset($bank_buyer->nama_bank)) {
							echo $bank_buyer->nama_bank;
						}?></td>
					</tr>
					<tr>
						<th>Account Number</th>
						<td>:</td>
						<td><?php if (isset($bank_buyer->nomor)) {
							echo $bank_buyer->nomor;
						}?></td>
					</tr>
					</table>
				<?php } else { ?>
			...
			<div class="block">
				...
				<address>
					<h4>
					<strong><?=$order->shipping->nama; ?>
						<small  style="display: none;">
							<a href="https://api.whatsapp.com/send?phone=<?=$order->nation_code.$order->shipping->telp; ?>" target="_blank">
			...
			<table class="table table-bordered table-vcenter">
				<tbody>
					...
					<tr>
						...
						<td><a href="#"><?=ucfirst($proses->initiator)?></a></td>
