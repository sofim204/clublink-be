...
$(document).ready(function(){
	if(jQuery('#drTable').length>0){
		...
		.on('preXhr.dt', function ( e, settings, data ){
			...
			"bServerSide"		: true,
			//"searching"			: true,
			"sAjaxSource"		: "<?=base_url("api_admin/ecommerce/blacklistuser/"); ?>",
		...
		$('.dataTables_filter input').attr('placeholder', 'Search FCM Token').css({'width':'250px', 'display':'inline-block'}); <!-- show searchbox + add styling -->
	}


	$("#add_data").on("click", function(e){
		e.preventDefault();
		$("#modal_add_blacklist").modal("show");
	});

	$("#modal_add_blacklist").on("hidden.bs.modal",function(e){
		$("#modal_add_blacklist").find("form").trigger("reset");
	});

	$("#form_add_blacklist").on("submit",function(e){

		let type_blacklist = $("#itype").val();
		let text = $("#itext").val();
		var ipformat = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;

		//if(type_blacklist == "fcm_token" && text.length <= 40) {
		//	alert("minimal 40 character");
		//} else 
		if(type_blacklist == "ip_address" && !text.match(ipformat)) {
			alert("invalid ip address");
		} else {
			e.preventDefault();
			var fd = new FormData($(this)[0]);
			var url = '<?=base_url("api_admin/ecommerce/blacklistuser/tambah/")?>';
			NProgress.start();
			$.ajax({
				...
				success: function(respon){
					...
					if(respon.status==200){
						...
						$("#modal_add_blacklist").modal("hide");
	...
	$("#btn_delete_data").on("click",function(e){
		...
		if(c){
			var url = '<?=base_url('api_admin/ecommerce/blacklistuser/hapus/')?>'+ieid;
			NProgress.start();
