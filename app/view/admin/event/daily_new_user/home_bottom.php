			...
			"fnServerData"		: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						NProgress.start();
						var status = $('#drTable').DataTable().row(currentRow).data()[15];
						var cdate3 = $('#drTable').DataTable().row(currentRow).data()[13];
...
$("#b_edit_telp").on("click", (e) => {
	...
	var numbers = /^[0-9]+$/;
		...
		if(confirm_message) {
			...
			{
				NProgress.start();
				var url = '<?=base_url() ?>api_admin/event/daily_new_user/edit_telp/?id=' +ieid+ '&telp=' +telp;
				$.get(url).done(function(response){
					NProgress.done();
					if(response.status=="200"){
...
$("#b_verif_telp").on("click", (e) => {
	...
	if(confirm_message) {
		NProgress.start();
		var url = '<?=base_url() ?>api_admin/event/daily_new_user/verif_telp/?id=' +ieid;
		$.get(url).done(function(response){
			NProgress.done();
			if(response.status=="200"){
				gritter('<h4>Success</h4><p>Successfully Accepted</p>','success');
...
$("#b_rejected_note").on("click", (e) => {
    ...
    if(confirm_message) {
		if (note != '') {
			NProgress.start();
			var url = '<?=base_url() ?>api_admin/event/daily_new_user/rejected/?id=' +ieid+ '&note=' +note;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status=="200"){
...
$("#b_finished").on("click", (e) => {
    ...
    if(confirm_message) {
        NProgress.start();
        var url = '<?=base_url() ?>api_admin/event/daily_new_user/finished/?id=' +ieid;
        $.get(url).done(function(response){
            NProgress.done();
            if(response.status=="200"){
