...
var ieid = '';
var url_def = '';
var type_event_banner = ''; <!-- by Muhammad Sofi 17 January 2022 11:06 | add logic if banner is video, hide input description -->
...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		"bProcessing"		: true,
		"bServerSide"		: true,
		"sAjaxSource"		: "<?=base_url("api_admin/event_banner/"); ?>",
		...
		"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
			oSettings.jqXHR = $.ajax({
				...
				$('#drTable > tbody').on('click', 'tr', function (e) {
					...
					ieid = id
					var url = '<?=base_url("api_admin/event_banner/detail/")?>' + ieid;
					$.get(url).done(function(response){
						if(response.status==200){
							var dta = response.data;
							$("#ieid").val(dta.id);
							$("#iejudul").val(decodeHtml(dta.judul)); <!-- by Muhammad Sofi 3 January 2022 18:23 | add title for event banner -->
							...
							if(dta.url_type == "video") {
								$("#modal_options").modal("show");
								$("#imageDisplay").attr("src","<?=base_url()?>"+dta.img_thumbnail);
								$('#videopreviewedit source').attr('src', '<?=base_url()?>'+dta.url);
								$("#videopreviewedit")[0].load();
							}

							<!-- check if event banner is video or photo before show edit modal -->
							var url_type = dta.url_type; 
							type_event_banner = url_type; <!-- by Muhammad Sofi 17 January 2022 11:06 | add logic if banner is video, hide input description -->
							...
							//$("#itype_event_banner_edit").val(dta.type_event_banner);
							$("#ietype_value_edit").val(dta.type_event_banner);
							
							var type = dta.type_event_banner;
...
<!-- by Muhammad Sofi 7 January 2022 19:57 | on exit modal, if preview video still playing, pause the video -->
$("#modal_options").on('hidden.bs.modal', function (e) {
	//document.getElementById("video_loader").pause();
	$("#video_loader")[0].pause();
});
...
$("#aEditBanner").on("click",function(e){
	...
	setTimeout(function(){
		...
		} else if(type_event_banner == "image") {
...
$("#atestURL").on("click",function(e){
	e.preventDefault();
	$("#modal_options").modal("hide");
	window.open('<?=base_url_admin('event_banner/getDetailEventBanner/')?>'+ieid, '_blank');
});
...
$("#ftambah").on("submit",function(e){
	$('#iteks').val(jqueryA('#iteks').Editor('getText')); <!-- by Muhammad Sofi 3 January 2022 17:18 | add description for event banner -->
	e.preventDefault();
	NProgress.start();
	var fd = new FormData($("#ftambah")[0]);
	console.log(fd);
	var url = '<?=base_url("api_admin/event_banner/tambah/")?>';
	$.ajax({
		url: url,
		type: 'POST',
		...
		if(respon.status==200){
			Swal.fire({
				title: 'Event Banner has been added successfuly',
				icon: 'success',
			...
			$('#ftambah')[0].reset();
			jqueryA('#iteks').Editor('setText',$("#iteks").val(""));
			$("#chooseFile").hide();
			...
			<!-- End by Muhammad Sofi 7 January 2022 21:43 | after save data, clear value form data in modal -->
			//gritter( '<h4>Success</h4><p>Event Banner has been added successfuly</p>','success');
		}else{
...
$("#ftedit").on("submit",function(e){
	...
	var url = '<?=base_url("api_admin/event_banner/edit/")?>'+ieid;
	$.ajax({
		url: url,
		type: 'POST',
		mimeType : "multipart/form-data",
		...
		if(respon.status==200){
			...
			drTable.ajax.reload(null, false);
			//setTimeout(function(){
			//	window.location = '<?php echo base_url_admin('event_banner'); ?>';
			//}, 600);
		}else{
			//gritter('<h4>Failed</h4><p>'+respon.message+'</p>','danger');
			Swal.fire({
	...
	if(ieid){
		Swal.fire({
			...
			if (result.isConfirmed) {
				var url = '<?=base_url('api_admin/event_banner/hapus/')?>'+ieid;
				$.get(url).done(function(response){
					$("#modal_edit").modal("hide");
					if(response.status==200){
...
$("#fthumbnail_change").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	var url = '<?=base_url("api_admin/event_banner/change_thumbnail/")?>'+ieid;
	$.ajax({
...
$("#itype_event_banner").change(function(){
	var type_event_banner = $("#itype_event_banner").val();
	if(type_event_banner == "original" || type_event_banner == "polling" || type_event_banner == "webview") {
		$("#row_url").hide();
		$("#row_seller_shop").hide();
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
	...
	} else if(type_event_banner == "shop") {
		$("#row_url").hide();
		$("#input_url_webview").val("");
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "product") {
		$("#row_url").hide();
		$("#input_url_webview").val("");
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "community") {
		$("#row_url").hide();
		$("#input_url_webview").val("");
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "webview_wallet") {
		$("#row_url").show();
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
		$("#row_seller_shop").hide();
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "guide") {
		$("#row_url").hide();
		$("#input_url_webview").val("");
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "activity_dashboard") {
		$("#row_url").hide();
		$("#input_url_webview").val("");
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "invitation_page") {
		$("#row_url").show();
		$("#input_url_webview").val("");
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
		...
		$("#select_community").val("").trigger("change");
		$("#input_description").hide();
	} else if(type_event_banner == "babyboom") {
		$("#row_url").show();
		$("#input_url_webview").val("");
		$("#row_seller_shop_product").hide();
		$("#row_product_detail").hide();
...
$("#select_seller_shop").select2({
	...
	ajax: { 
		url: "<?= base_url('api_admin/event_banner/getCustomer') ?>",
		type: "post",
		dataType: 'json',
		delay: 250,
...
$("#select_product_detail").select2({
	...
	allowClear: true, <!-- add x button to clear value -->
	ajax: { 
		url: "<?= base_url('api_admin/event_banner/getProductDetail') ?>",
		type: "post",
		dataType: 'json',
		delay: 250,
		data: function (params) {
...
$("#select_community").select2({
	...
	allowClear: true, <!-- add x button to clear value -->
	ajax: { 
		url: "<?= base_url('api_admin/event_banner/getCommunity') ?>",
		type: "post",
		dataType: 'json',
		delay: 250,
		data: function (params) {
