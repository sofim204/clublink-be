...
$(document).ready(function(){
	if(jQuery('#drTable').length>0){
		...
		.on('preXhr.dt', function ( e, settings, data ){
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/misc/exceptionaluserid/"); ?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						//var user_id_sg = $('#drTable').DataTable().row(currentRow).data()[2];
						var user_id_id = $('#drTable').DataTable().row(currentRow).data()[2];
						ieid = id;

						//$("#edit_user_id_sg").val(user_id_sg)
						$("#edit_user_id_id").val(user_id_id)
	...
	$("#form_add_data").on("submit",function(e){
		...
		var url = '<?=base_url("api_admin/misc/exceptionaluserid/add/")?>';
		$.ajax({
	...
	$("#form_edit_data").on("submit",function(e){
		...
		var url = '<?=base_url("api_admin/misc/exceptionaluserid/edit/")?>' + ieid;
		$.ajax({
			url: url,
			type: 'POST',
	...
	$("#bdelete").on("click",function(e){
		...
		if(ieid){
			...
			if(c){
				NProgress.start();
				var url = '<?=base_url('api_admin/misc/exceptionaluserid/delete/')?>'+ieid;
