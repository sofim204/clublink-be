...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/misc/domainchanger/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var url = '<?=base_url("api_admin/misc/domainchanger/detail/")?>'+ieid;
	...
	$('.dataTables_filter input').attr('placeholder', 'Search domain');
...
$("#modal_tambah").on("shown.bs.modal",function(e){
	...
	$("#ftambah").on("submit",function(e){
		...
		var url = '<?=base_url('api_admin/misc/domainchanger/tambah/');?>';
		$.ajax({
			type: 'post',
			url: url,
			data: fd,
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/misc/domainchanger/edit/"); ?>';
	$.ajax({
...
$("#fpe").on("submit",function(e){
	...
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/misc/domainchanger/editpass/"); ?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?=base_url('api_admin/misc/domainchanger/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status=="200" || response.status==200){
...
function popUp() {
  ...
  var url = '<?=base_url("api_admin/misc/domainchanger/security/"); ?>';
  if (pin) {
	$.ajax({
        url: url,
        type: 'POST',
