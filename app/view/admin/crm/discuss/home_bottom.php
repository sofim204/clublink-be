...
var api_url = '<?=base_url('api_admin/crm/discuss'); ?>';
var drTable = {};
var ieid = '';
App.datatables();
...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/crm/discuss"); ?>",
...
$("#bfilter").on("click",function(e){
  ...
  e.preventDefault();
  window.location='<?=base_url_admin('crm/discuss/reported')?>';
});
...
$("#adetail").on("click",function(e){
	...
	$("#modal_option").modal("hide");
	setTimeout(function(){
		window.location='<?=base_url_admin('crm/discuss/default/')?>'+ieid;
	},333);
});
