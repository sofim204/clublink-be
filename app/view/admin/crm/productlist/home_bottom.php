...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/crm/productlist")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var user_id = $('#drTable').DataTable().row(currentRow).data()[9];
						var email_creator = $('#drTable').DataTable().row(currentRow).data()[10];
						var name_creator = $('#drTable').DataTable().row(currentRow).data()[11];
						var status_product = $('#drTable').DataTable().row(currentRow).data()[8]; // is_active
						var get_status = extractContent(status_product);
						...
						if(get_status == "Inactive") {
							...
							$("#b_restore_product").show();
						} else if(get_status == "Publish Active " || get_status == "Draft"){
							$("#b_delete_product").show();
							$("#b_restore_product").hide();
						} else {}
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?=base_url("api_admin/crm/productlist/tambah/")?>';
	$.ajax({
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/crm/productlist/edit/")?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			var url = '<?=base_url('api_admin/crm/productlist/hapus/')?>'+ieid;
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?=base_url_admin('crm/productlist/edit/')?>'+ieid;
	},333);
});
...
$("#adetail").on("click",function(e){
	...
	setTimeout(function(){
		...
		//window.location ='<?=base_url_admin('crm/productlist/detail/')?>'+ieid;
		window.open("<?=base_url_admin('crm/productlist/detail/')?>"+ieid, "_blank");
	},333);
});
...
$("#aset_showhp").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/homepage_show/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#aset_hidehp").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/homepage_hide/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#aset_publish").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/publish/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#aset_draft").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/draft/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#aset_active").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/active/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#aset_inactive").on("click",function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/crm/productlist/inactive/")?>"+ieid).done(function(dt){
		NProgress.done();
		if(dt.status == 200){
...
$("#b_delete_product").on("click", (e) => {
	...
	if(confirm_message) {
		...
		var url = '<?=base_url() ?>api_admin/crm/productlist/delete_from_admin/?c_product_id=' +ieid+'&admin_name='+admin_name;
		$.get(url).done(function(response){
			NProgress.done();
			if(response.status=="200"){
...
$("#b_restore_product").on("click", (e) => {
    e.preventDefault();
    let admin_name = $("#admin_name").text();
    let confirm_message = confirm("Are you sure to restore this product?");
    if(confirm_message) {
        NProgress.start();
        var url = '<?=base_url() ?>api_admin/crm/productlist/restore_from_admin/?c_product_id=' +ieid+'&admin_name='+admin_name;
        $.get(url).done(function(response){
            NProgress.done();
            if(response.status=="200"){
                gritter('<h4>Success</h4><p>Product Successfully Restored</p>','success');
                setTimeout(function(){
                    drTable.ajax.reload(null, false);
					$("#modal_option").modal("hide");
                }, 400);
