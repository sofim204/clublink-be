...
$(document).ready(function(){
	if(jQuery('#drTable').length>0){
		...
		.on('preXhr.dt', function ( e, settings, data ){
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/cms/multilanguage/"); ?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						var url = '<?=base_url("api_admin/cms/multilanguage/detail/")?>'+id;
						$.get(url).done(function(response){
							if(response.status==200){
								var dta = response.data;
								$("#ieid").val(dta.id);
								$("#ievariable_name").val(dta.variable_name);
								$("#ieindonesia").val(dta.indonesia);
								$("#ieenglish").val(dta.english);
								$("#iekorea").val(dta.korea);
								$("#iethailand").val(dta.thailand);
	...
	$("#form_add_data").on("submit",function(e){
		...
		var url = '<?=base_url("api_admin/cms/multilanguage/add/")?>';
		$.ajax({
	...
	$("#form_edit_data").on("submit",function(e){
		...
		var url = '<?=base_url("api_admin/cms/multilanguage/edit/")?>' +ieid;
		$.ajax({
			url: url,
			type: 'POST',
	...
	$("#bdelete").on("click",function(e){
		...
		if(ieid){
			...
			if(c){
				NProgress.start();
				var url = '<?=base_url('api_admin/cms/multilanguage/delete/')?>'+ieid;
				$.get(url).done(function(response){
					NProgress.done();
					$("#modal_edit").modal("hide");
					if(response.status==200){
	...
	$("#fltype_multilanguage").change(function(){
		drTable.ajax.reload();
	});

	$("#reset-filter").on("click",function(e){
		e.preventDefault();
		$("#fltype_multilanguage").val("");
		drTable.search('').columns().search('').draw();
		drTable.ajax.reload();
	});
});