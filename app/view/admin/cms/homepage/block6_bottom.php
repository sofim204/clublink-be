...
function saveHomePageData(){
	...
	var fd = {};
	fd.block6_enable = $("#ihp_block6_enable").val();
	fd.block6_teks = $("#ihp_block6_teks").val();
	fd.block6_image = '';

	var items = $('#block6_items input[name="image[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block6_image = $(this).val();
		});
	}

  $.post('<?=base_url('api_admin/cms/homepage/block6_data/')?>',fd).done(function(dt){
		NProgress.done();
