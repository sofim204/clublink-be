...
function saveHomePageData(){
	...
	var fd = {};
	fd.mtitle = $("#ihpmtitle").val();
	fd.mkeyword = $("#ihpmkeyword").val();
	fd.mdescription = $("#ihpmdescription").val();
	fd.slider_enable = $("#ihpslider_enable").val();

	var img = '';
	var imgs = $('#slider_items input[name="image[]"]');
	$.each(imgs,function(k,v){
		img += $(this).val()+',';
	});
	if(img.length>0) img = img.slice(0, -1);
	fd.slider_list = img;

	var img = '';
	var imgs = $('#block1_items input[name="image[]"]');
	$.each(imgs,function(k,v){
		img += $(this).val()+',';
	});
	if(img.length>0) img = img.slice(0, -1);
  ...
  $.post('<?=base_url('api_admin/cms/homepage/data/')?>',fd).done(function(dt){
		NProgress.done();
