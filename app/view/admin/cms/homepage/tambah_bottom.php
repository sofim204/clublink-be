...
function genSku(){
	var n = $("#inama").val().toUpperCase().trim().replace(/[^\w\s]/gi, '');
	var k = $("#ib_kategori_id option:selected").attr('data-kode').trim().toUpperCase();
	var t = $("#itindakan_oleh option:selected").html().trim().slice(0,1).toUpperCase();
	var a = $("#iis_asistensi").val();
	var s = $("#iscope option:selected").attr("data-kode").toUpperCase();
	var c = $("#ia_company_id option:selected").attr("data-kode").toUpperCase();
	...
	if(s.toLowerCase() != 'al' && c.toLowerCase() != '00'){
		$("#isku").val(c+'.'+k+'.J'+t+a+n);
	}else{
		$("#isku").val(k+'.J'+t+a+n);
	}

}
$("#ia_company_id").on("blur",function(e){
	e.preventDefault();
	$("#iscope").val('all');
	if($(this).val().toLowerCase() != 'null'){
		$("#iscope").val('current_only');
	}
	genSku();
});
$("#iscope").on("blur",function(e){ e.preventDefault(); genSku(); });
$("#inama").on("blur",function(e){ e.preventDefault(); genSku(); });
$("#ib_kategori_id").on("blur",function(e){ e.preventDefault(); genSku(); });
$("#itindakan_oleh").on("blur",function(e){ e.preventDefault(); genSku(); });
$("#iis_asistensi").on("blur",function(e){ e.preventDefault(); genSku(); });
//$("#isku").on("blur",function(e){ e.preventDefault(); genSku(); });
...
$("#ftambah").on("submit",function(e){
	...
	NProgress.start();
	updateCkEditor();
	$("#iproduk_komposisi").val(JSON.stringify(produk_terpilih));
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url('api_admin/ecommerce/flashsale/tambah/'); ?>';
	$.ajax({
		...
		success: function(respon){
			if(respon.status=="100" || respon.status == 100){
				gritter('<h4>Berhasil</h4><p>Proses tambah data telah berhasil!</p>','success');
				setTimeout(function(){
					NProgress.done();
