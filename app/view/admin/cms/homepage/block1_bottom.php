...
function saveHomePageData(){
	...
	var fd = {};
	fd.block1_enable = $("#ihp_block1_enable").val();
	fd.block1_mode = $("#ihp_block1_mode").val();
	fd.block1_images = [];
	fd.block1_captions = [];
	fd.block1_urls = [];

	var items = $('#block1_items input[name="image[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block1_images.push($(this).val());
		});
	}

	var items = $('#block1_items input[name="caption[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block1_captions.push($(this).val());
		});
	}

	var item = '';
	var items = $('#block1_items input[name="targeturl[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block1_urls.push($(this).val());
		});
	}

  $.post('<?=base_url('api_admin/cms/homepage/block1_data/')?>',fd).done(function(dt){
		NProgress.done();
