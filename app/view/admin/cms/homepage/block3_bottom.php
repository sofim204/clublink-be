...
function saveHomePageData(){
	...
	var fd = {};
	fd.block3_enable = $("#ihp_block3_enable").val();
	fd.block3_teks = $("#ihp_block3_teks").val();
	fd.block3_images = [];

	var items = $('#block3_items input[name="image[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block3_images.push($(this).val());
		});
	}


  $.post('<?=base_url('api_admin/cms/homepage/block3_data/')?>',fd).done(function(dt){
		NProgress.done();
