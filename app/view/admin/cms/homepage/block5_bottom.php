...
function saveHomePageData(){
	...
	var fd = {};
	fd.block5_enable = $("#ihp_block5_enable").val();
	fd.block5_teks = $("#ihp_block5_teks").val();
	fd.block5_images = [];

	var items = $('#block5_items input[name="image[]"]');
	if(items.length>0){
		$.each(items,function(k,v){
			fd.block5_images.push($(this).val());
		});
	}

  $.post('<?=base_url('api_admin/cms/homepage/block5_data/')?>',fd).done(function(dt){
		NProgress.done();
