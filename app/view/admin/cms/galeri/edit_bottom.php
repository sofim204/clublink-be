...
var folder_id = '';
var media_name = '';
var galeri_item_count = <?php echo count($blog->items); ?>;
...
function row_media_manager(){
  	...
  	var url = '<?php echo base_url('api/media/'); ?>';
  ...
  $("#mfaddform").on("submit",function(e){
  	...
  	$.ajax({
  		url: '<?php echo base_url('api/media/add'); ?>', // Url to which the request is send
  		type: "POST",
  ...
  $("#afeditsubmit").on("click",function(e){
    e.preventDefault();
    $("#fedit").submit();
  })
  ...
  $("#fedit").on("submit",function(e){
    ...
    tinymce.triggerSave();
    var frm = $(this);
    var url = "<?php echo base_url("api/galeri/update/"); ?>"+id;
    $.ajax({
      ...
      error: function(){
        jQuery.gritter.add({
          title: 'Error',
          text: 'Untuk saat ini tidak dapat menyimpan galeri, silakan coba beberapa saat lagi',
          //class_name: 'growl-info',
          image: '<?php echo base_url('skin/admin/'); ?>images/comment.png',
          sticky: false,
          time: ''
