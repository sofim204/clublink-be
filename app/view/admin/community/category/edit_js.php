...
$("#fedit").on("submit",function(e){
	...
	var url = '<?=base_url("api_admin/community/category/edit/".$kategori_data->id); ?>';
...
$("#modal_media_add_form").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	gritter('<h4>Loading..</h4><p>Please wait uploading image...</p>','info');
	...
	$.ajax({
		...
		success: function(data){
			if(data.status == "100" || data.status == 100){
				setTimeout(function(){
					gritter('<h4>Success</h4><p>File has been uploaded successfuly!</p>','success');
				},1333);
			}else{
		...
		error: function(d){
			NProgress.done();
			gritter('<h4>Error</h4><p>Cant upload image right now, please try again later</p>','danger');
		}
	});
...
foreach($katdata as $ky=>&$kv){
	// by Muhammad Sofi 21 January 2022 16:11 | read special character like &#38 ;
	if($ky == 'nama' || $ky == 'indonesia' || $ky == 'korea' || $ky == 'thailand') {
		$kv = html_entity_decode($kv, ENT_QUOTES);
	}
	if($ky == 'deskripsi' || $ky == 'deskripsi_indonesia' || $ky == 'deskripsi_korea' || $ky == 'deskripsi_thailand'){
		$kv = html_entity_decode($kv, ENT_QUOTES);
	}
}
?>
