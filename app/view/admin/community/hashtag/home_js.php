...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/community/hashtag/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id	
						var url = '<?=base_url()?>api_admin/community/hashtag/detail/'+id;
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?=base_url("api_admin/produk/hashtag/tambah/"); ?>';
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/community/hashtag/hapus/'); ?>'+ieid;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status==200){
