...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/community/likes/"); ?>",
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?=base_url("api_admin/produk/likes/tambah/"); ?>';
...
$("#modal_edit").on("shown.bs.modal",function(e){
	//
	var url = '<?=base_url(); ?>api_admin/produk/likes/detail/'+ieid;
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/produk/likes/edit/"); ?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/community/likes/hapus/'); ?>'+ieid;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status==200){
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?=base_url_admin('community/likes/edit/');?>'+ieid;
	},333);
});
...
$("#ficon_change").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	var url = '<?=base_url("api_admin/community/likes/change_icon/")?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
...
$("#adetail").on("click",function(e){
	...
	setTimeout(function(){
		//$("#modal_edit").modal("show");
		//alert('masih dalam pengembangan');
		window.location ='<?=base_url_admin('community/likes/detail/')?>'+ieid;
	},333);
});
...
$("#areport").on('click',function(e){
	e.preventDefault();
	NProgress.start();
	$.get("<?=base_url("api_admin/community/likes/report")?>"+ieid).done(function(dt){
		NProgress.done();
		$("#modal_option").modal("hide");
