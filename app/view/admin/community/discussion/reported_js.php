...
<!-- by Muhammad Sofi 14 January 2022 18:09 | move function to get function data from community/discussion to community/listing -->
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			//"sAjaxSource"		: "<?=base_url("api_admin/community/discussion/reported"); ?>",
			"sAjaxSource"		: "<?=base_url("api_admin/community/listing/reported_discussion"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var id = $('#drTable').DataTable().row(currentRow).data()[0];
						var com_discuss_id = $('#drTable').DataTable().row(currentRow).data()[1];
						ieid = id;
						community_discussion_id = com_discuss_id;
	...
	$('.dataTables_filter input').attr('placeholder', 'Search reported discussion').css({'width':'250px', 'display':'inline-block'}); <!-- show searchbox + add styling -->
}
...
$("#atakedown").on('click',function(e){
	e.preventDefault();
	NProgress.start();
	//$.get("<?=base_url("api_admin/community/discussion/takedown/")?>"+ieid).done(function(dt){
	<!-- is_takedown set to 1 -->
	var url = '<?=base_url() ?>api_admin/community/listing/report_discussion/1/?id=' +ieid+'&c_community_discussion_id='+community_discussion_id;
	$.get(url).done(function(dt){
		NProgress.done();
		$("#modal_option").modal("hide");
		drTable.ajax.reload(null,false);
		if(dt.status == "200"){
			gritter("<h4>Success</h4><p>Discussion Taken down.</p>",'success');
