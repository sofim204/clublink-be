...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/band/group_report/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						$("#bconfirmation").hide();
						$("#btakedown").hide();
						var verif_telp = $('#drTable').DataTable().row(currentRow).data()[16];
						if (verif_telp == 1) {
							$("#btakedown").show();
						}else{
							$("#bconfirmation").show();
...
//confirmation
$("#bconfirmation").on("click",function(e){
	e.preventDefault();
	if(ieid){
		var c = confirm('Are you sure already confirmation this data?');
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/band/group_report/confirmation/'); ?>'+ieid;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status==200){
					gritter('<h4>Success</h4><p>Data successfully confirmation</p>','success');
				}else{
				...
				NProgress.done();
				gritter('<h4>Error</h4><p>Cant confirmation data right now, please try again later</p>','danger');
			});
		}
	}
});
...
//restore
$("#brestore").on("click",function(e){
	e.preventDefault();
	if(ieid){
		var c = confirm('Are you sure restore this data?');
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/band/group_report/restore/'); ?>'+ieid;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status==200){
					gritter('<h4>Success</h4><p>Data successfully restore</p>','success');
				}else{
					gritter('<h4>Failed</h4><p>'+response.message+'</p>','danger');
				}
				drTable.ajax.reload();
				...
				NProgress.done();
				gritter('<h4>Error</h4><p>Cant restore data right now, please try again later</p>','danger');
			});
		}
	}
});
...
$("#btakedown").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/band/group_report/takedown/'); ?>'+ieid;
			$.get(url).done(function(response){
				NProgress.done();
				if(response.status==200){
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?=base_url_admin('band/group_report/edit/');?>'+ieid;
	},333);
});
...
$("#adetail").on("click",function(e){
	...
	$("#modal_option").modal("hide");
	setTimeout(function(){
		window.location='<?=base_url_admin('band/group_report/detail_group_post/')?>'+ieid;
	},333);
});
...
$("#ficon_change").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	var url = '<?=base_url("api_admin/band/group_report/change_icon/")?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
