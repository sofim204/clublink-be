...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/band/homelist/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						var type_homelist = $('#drTable').DataTable().row(currentRow).data()[9]; <!-- to get data from specific column, change this "data()[id_column]" -->
						ieid = id;
						gettype = type_homelist;

						if(type_homelist != "sub_category") {
							$("#aicon_change").addClass("hidden")
						} else {
							$("#aicon_change").removeClass("hidden")
						}
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			NProgress.start();
			var url = '<?=base_url('api_admin/band/homelist/hapus/'); ?>'+ieid;
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?=base_url_admin('band/homelist/edit_page/');?>'+ieid + '/' + gettype;
	},333);
});
...
$("#ficon_change").on("submit",function(e){
	e.preventDefault();
	NProgress.start();
	var url = '<?=base_url("api_admin/band/homelist/change_icon/")?>'+ieid;
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		...
		success: function(respon){
			if(respon.status==200){
				gritter('<h4>Success</h4><p>Image changed successfully</p>','success');
				setTimeout(function(){
					NProgress.done();
					drTable.ajax.reload();
