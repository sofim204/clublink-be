...
class Convert_video_community extends JI_Controller
{
    ...
    public function index()
    {
        ...
        $this->seme_log->write("api_cron", 'API_Cron/Convert_Video_Community::index --start');

        $nation_code = 62;

        $videoinprocessing = $this->ccam->getAll($nation_code, "video", "processing");
        if($videoinprocessing){

            $this->seme_log->write("api_cron", 'API_Cron/Convert_Video_Community::index --not doing convert because there is video still processing convert');

        }else{

            $videoNeedConvert = $this->ccam->getAll($nation_code, "video", "waiting", "", "no", 2);
            if($videoNeedConvert){
                ...
                $this->seme_log->write("api_cron", 'API_Cron/Convert_Video_Community::index --not doing convert because there is no video in waiting');
        ...
        $this->seme_log->write("api_cron", 'API_Cron/Convert_Video_Community::index --stop');
