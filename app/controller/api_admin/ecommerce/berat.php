...
class Berat extends JI_Controller{
    ...
    parent::__construct();
		...
		$this->load("api_admin/b_berat_model",'bbem');
	}
	...
	public function index(){
		...
		$dcount = $this->bbem->countAll($nation_code, $keyword);
		$ddata = $this->bbem->getAll($nation_code, $page,$pagesize,$sortCol,$sortDir,$keyword);
    ...
    //$this->debug($d);
		...
		if(strlen($di['nama'])>0 && strlen($di['nilai'])>0){
			$this->bbem->trans_start();
			$berat_id = $this->bbem->getLastId($nation_code);
	    $di['nation_code'] = $nation_code;
	    $di['id'] = $berat_id;
      $res = $this->bbem->set($di);
			if($res){
				$this->bbem->trans_commit();
				$this->status = 200;
				...
				if(isset($_FILES['icon'])){
					...
					$target_file = "berat-$nation_code-$berat_id.$ext";
					...
					if($mv){
						$du = array();
						$du['icon'] = $filename;
						$res = $this->bbem->update($nation_code, $berat_id, $du);
						if($res){
							$this->bbem->trans_commit();
						}
					}
	...
	public function detail($id){
		...
		$data = $this->bbem->getById($nation_code, $id);
		if(!isset($data->id)){
			$this->status = 400;
    ...
    $nation_code = $pengguna->nation_code;
		...
		if($id>0 && strlen($du['nama'])>0 && strlen($du['nilai'])>0){
			$res = $this->bbem->update($nation_code,$id,$du);
			if($res){
				$this->status = 200;
    ...
    $berat = $this->bbem->getById($nation_code, $id);
    if(isset($berat->id)){
      $res = $this->bbem->del($nation_code,$id);
  		if($res){
				$this->status = 200;
				$this->message = 'Success';
			}else{
        ...
        //hapus icon
        if(strlen($berat->icon)>6){
          if(file_exists(SENEROOT.$berat->icon)){
            unlink(SENEROOT.$berat->icon);
          }
        }
  		}
    }else{
	...
	public function change_icon($id){
		...
		if(isset($_FILES['icon'])){
			...
			$target_dir = $this->media_icon;
			$target_file = "berat-$nation_code-$id.$ext";
			$filename = $target_dir.$target_file;

			if(file_exists(SENEROOT.$filename)){
				unlink(SENEROOT.$filename);
				$target_file = "berat-$nation_code-$id".rand(0,999).".$ext";
				$filename = $target_dir.$target_file;
			...
			if($mv){
				$du = array();
				$du['icon'] = $filename;
				$res = $this->bbem->update($nation_code, $berat->id,$du);
				if($res){
					$this->status = 200;
					$this->message = 'Success';
					if(strlen($berat->icon)>4){
						if(file_exists(SENEROOT.DIRECTORY_SEPARATOR.$berat->icon)) unlink(SENEROOT.DIRECTORY_SEPARATOR.$berat->icon);
					}
				}else{
					$this->status = 1034;
