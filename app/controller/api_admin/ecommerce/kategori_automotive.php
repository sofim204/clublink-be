...
class kategori_automotive extends JI_Controller {
	public function __construct(){
		...
		$this->load("api_admin/b_kategori_automotive_model",'bkam');
	...
	public function index(){
		...
		$dcount = $this->bkam->countAll($nation_code, $keyword);
		$ddata = $this->bkam->getAll($nation_code, $page, $pagesize, $sortCol, $sortDir, $keyword);
		...
		foreach($ddata as &$gd){
			...
			if(isset($gd->parent_b_kategori_id)){
				if($gd->parent_b_kategori_id == 32){
					$gd->parent_b_kategori_id = 'Car';
				}else if ($gd->parent_b_kategori_id == 33){
					$gd->parent_b_kategori_id = 'Motorcycle';
				}
			}
	...
	public function tambah(){
		...
		//start transaction
		$this->bkam->trans_start();
		//get last id
		$bkam_id = $this->bkam->getLastId($pengguna->nation_code);
		...
		$di['image_icon'] = '';
        $di['utype'] = 'brand';
        $di['parent_nation_code'] = $pengguna->nation_code;
		//insert into db
		$res = $this->bkam->set($di);
		if($res){
			$this->bkam->trans_commit();
			$this->status = 200;
			...
			$target_file = $this->__slugify($di['nama'])."$pengguna->nation_code-$bkam_id-".date("His").".$ext";
			...
			$res2 = $this->bkam->update($pengguna->nation_code, $bkam_id, $dux);
			if($res2){
				$this->status = 200;
				$this->message = 'Success';
				$this->bkam->trans_commit();
	...
	public function edit($id){
		...
		$kategori = $this->bkam->getById($nation_code, $id);
		...
		$res = $this->bkam->update($pengguna->nation_code, $id, $du);
		if($res){
			$this->status = 200;
			$this->message = 'Success';
		}else{
	...
	public function hapus($id){
		...
		$pengguna = $d['sess']->admin;
		$kategori = $this->bkam->getById($pengguna->nation_code, $id);
		if(!isset($kategori->id)){
			$this->status = 520;
		...
		$res = $this->bkam->del($pengguna->nation_code, $id); // for delete
		if($res){
			if(strlen($kategori->image_icon)>4){
                // if it set active to 0, remark code below
	...
	public function change_icon($id){
		...
		if($mv){
			...
			$this->bkam->update($pengguna->nation_code, $kategori->id, $du);
			$this->status = 200;
			$this->message = 'Success';
		}
		$this->__json_out($data);
