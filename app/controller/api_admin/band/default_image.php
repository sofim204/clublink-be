...
class Default_Image extends JI_Controller{
    ...
    parent::__construct();
		...
		$this->load("api_admin/i_group_default_image_model",'igdim');
	...
	public function index(){
		...
		$dcount = $this->igdim->countAll($nation_code,$keyword);
		$ddata = $this->igdim->getAll($nation_code,$page,$pagesize,$sortCol,$sortDir,$keyword);
	...
	public function tambah(){
		...
		//start transaction
		$this->igdim->trans_start();
		
        $default_image_id = $this->GUIDv4();
		...
		//insert into db
		$res = $this->igdim->set($di);
		if($res){
			$ext = 'jpg';
			$pi = pathinfo($_FILES['image_icon']['name']);
			if(isset($pi['extension'])) $ext = $pi['extension'];
			$target_dir = $this->media_group.'default_image/';
            //check folder
            if(!is_dir(SENEROOT.$target_dir)) mkdir(SENEROOT.$target_dir,0777);

			$target_file = $this->__slugify($default_image_id)."-$pengguna->nation_code-$default_image_id-".date("His").".$ext";
			...
			$dux = array();
			$dux['image'] = $filename;
			$res2 = $this->igdim->update($pengguna->nation_code, $default_image_id, $dux);
			if($res2){
				$this->status = 200;
				$this->message = 'Success';
				$this->igdim->trans_commit();
			}else{
				$this->status = 109;
				$this->message = 'Failed updating image icon';
				$this->igdim->trans_rollback();
				//delete file
				if(file_exists( SENEROOT.$filename)) unlink(SENEROOT.$filename);
			}
		}else{
	...
	public function detail($id){
		...
		$this->status = 200;
		$this->message = 'Success';
		$data = $this->igdim->getById($nation_code,$id);
		if(!isset($data->id)){
			$data = new stdClass();
			$this->status = 441;
	...
	public function edit($id){
		...
		$gd = $this->igdim->getById($nation_code, $id);
		if(!isset($gd->id)){
			$this->status = 111;
			$this->message = 'Wrong Default Image ID, please refresh this page';
			$this->__json_out($data);
			die();
		}
		...
		$res = $this->igdim->update($nation_code, $id, $du);
		if($res){
			$this->status = 200;
			$this->message = 'Success';
		}else{
			$this->status = 901;
			$this->message = 'Cant update default image to database';
		}
		$this->__json_out($data);
	}
	...
	// 	$gd = $this->igdim->getById($pengguna->nation_code, $id);
	// 	if(!isset($gd->id)){
	// 		$this->status = 111;
	...
	// 	$category_data = $this->igdim->getById($pengguna->nation_code, $id);
	...
	// 		die();
	// 	}
	// 	$res = $this->igdim->del($pengguna->nation_code, $id);
	// 	if($res){
	// 		if(strlen($category_data->image)>4){
	...
	public function change_icon($id){		
		...
		if($mv){
			if(strlen($gd->image)>4){
				$image = SENEROOT.DIRECTORY_SEPARATOR.$gd->image;
				if(file_exists($image)) unlink($image);
			}
			$du = array();
			$du['image'] = $filename;
			$this->igdim->update($pengguna->nation_code, $gd->id, $du);
			$this->status = 200;
			$this->message = 'Success';
		}
		$this->__json_out($data);
