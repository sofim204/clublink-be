...
class Group extends JI_Controller{
	...
	public function index(){
		...
		switch($iSortCol_0){
            ...
            case 7:
                ...
                break;
            case 8:
                $sortCol = "last_post_date"; // last post date
                break;
            case 9:
                $sortCol = "cdate";
                break;
		...
		$dcount = $this->igm->countAll($nation_code,$keyword);
		$ddata = $this->igm->getAll($nation_code,$page,$pagesize,$sortCol,$sortDir,$keyword);
		...
		foreach($ddata as &$gd){
			...
			// by Muhammad Sofi 15 February 2024 | show last post date
			if(isset($gd->last_post_date)) {
				$gd->last_post_date = $gd->last_post_date;
			} else {
				$gd->last_post_date = '0';
			}
		}
	...
	public function detail_group_post($id)
    {
		...
		foreach($ddata as &$gd){
			...
			if (isset($gd->url_thumb)) {
				...
				//get image thumbnail
				$image_data = $this->igm->getImageByPostId($nation_code, $gd->id);

				if(isset($image_data->url_thumb)) {
					$img_thumbnail = $image_data->url_thumb;
				} else {
	...
	public function check_user($user_id) {
		...
		$pengguna = $d['sess']->admin;
		$nation_code = $pengguna->nation_code;
		$data['join_in_which_club'] = $this->igm->getJoinedClubByUserId($nation_code, $user_id);

		foreach($data['join_in_which_club'] as $jiwc) {
			$count_total = $this->igm->countTotalPostHeJoinedInClub($nation_code, $jiwc->i_group_id, $user_id);
			$jiwc->count_total_post = $count_total;
			$club_name = $this->igm->getByIdByWith($nation_code, $jiwc->i_group_id);
			if(!isset($club_name->name) || $club_name->name == "") {
				$jiwc->club_name = "kosong";
			} else {
				$jiwc->club_name = $club_name->name;
			}
		}
	...
	public function popular_club(){
		...
		$this->status = 200;
		$this->message = 'Success';
		$dcount = $this->igm->countAllPopularClub($nation_code, $keyword, "popular");
		$ddata = $this->igm->getAllPopularClub($nation_code,$page,$pagesize,$sortCol,$sortDir,$keyword, "popular");
