...
class Popular_Club extends JI_Controller {
	...
	public function index(){
		...
		$dcount = $this->chmpm->countAllPopularClub($pengguna->nation_code, $keyword);
		$ddata = $this->chmpm->getAllPopularClub($pengguna->nation_code, $page, $pagesize, $sortCol, $sortDir, $keyword);
	...
	public function get_club() {
        ...
        $ddata = $this->chmpm->getClubList($nation_code, $search, 1);

        $data = array();

        foreach ($ddata as $gd) {
            $data[] = array("id"=>$gd->id, "text"=>$gd->name);
        }
        echo json_encode($data);
    }

	public function change_club($id) {
		$d = $this->__init();
        $pengguna = $d['sess']->admin;
		...
		$club_id = $_POST['custom_id'];
		// $checkdata = $this->chmpm->checkPopularClubAlreadyRegistered($club_id);
	...
	public function add_popular_club_to_homepage() {
		$d = $this->__init();
        $pengguna = $d['sess']->admin;
		...
		// 	die();
		// 	// $du = array();
		// 	// $du['custom_id'] = $club_id;
		// 	// $this->chmpm->updateBy($pengguna->nation_code, $_POST['priority'], $du);
		// }

		// check if there is same priority with start date end date
		$checkdata = $this->chmpm->checkDuplicatePriorityAndStartEndDate($pengguna->nation_code, $_POST['priority'], $_POST['start_date'], $_POST['end_date'], "club");
		if($checkdata > 0) {
			$this->status = 920;
	...
	// public function change_and_reorder_popular_club() {
	// 	$d = $this->__init();
    //     $pengguna = $d['sess']->admin;
