...
class Event_Banner extends JI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load("api_admin/c_event_banner_model", 'ceb');
    }
    ...
    public function index()
    {
        ...
        $dcount = $this->ceb->countAll($nation_code, $keyword, $is_active);
        $ddata = $this->ceb->getAll($nation_code, $page, $pagesize, $sortCol, $sortDir, $keyword, $is_active);
    ...
    public function tambah()
    {
        ...
        // by Muhammad Sofi 26 January 2022 11:28 | bug fix Undefined index: url
        $isuploadfile = $this->input->post("url_type");
        ...
        $di['url_webview'] = html_entity_decode($di['url_webview'], ENT_QUOTES);
        
        if (($di['type_event_banner']) == "") {
            $di['type_event_banner'] = "original";
        }
        ...
        if($isuploadfile == "image") {
            ...
            if ($fi["url"]["size"]>10120000) {
                $this->status = 1020;
                $this->message = 'Image file size too big, please try another image size';
                $this->seme_log->write("api_admin", "api_admin/Event_Banner::tambah() -> ".$this->message.". url_size ".$fi["url"]["size"]."bytes");
                $this->__json_out($data);
                die();
            } else if ($fi["url"]["size"] > 0 && $fi["url"]["size"] <= 10120000) {
                $this->seme_log->write("api_admin", "api_admin/Event_Banner::tambah() -> url_size ".$fi["url"]["size"]." bytes");
        ...
        if($isuploadfile == "video") {
            ...
            $dataVideo = $this->media_event_banner.'default.png';
            if ($fi["url"]["size"]>40960000) { // by Muhammad Sofi 20 January 2022 11:55 | request from mr jackie, change max file size upload to 40MB
                $this->status = 1020;
                $this->message = 'video file size too big, please try another video';
                $this->seme_log->write("api_admin", "api_admin/Event_Banner::tambah() -> ".$this->message.". url_size ".$fi["url"]["size"]."bytes");
                $this->__json_out($data);
                die();
            } else if ($fi["url"]["size"] > 0 && $fi["url"]["size"] <= 40960000) { // by Muhammad Sofi 20 January 2022 11:55 | request from mr jackie, change max file size upload to 40MB
                $this->seme_log->write("api_admin", "api_admin/Event_Banner::tambah() -> url_size ".$fi["url"]["size"]."bytes");
                $ext = strtolower(pathinfo($fi["url"]['name'], PATHINFO_EXTENSION));
                if (!in_array($ext, array("mp4", "mkv", "mov", "3gp"))) {
                    $this->status = 1026;
                    $this->message = 'Invalid video file extension, only allowed MP4 and MKV file format';
                    $this->__json_out($data);
                    die();
                }

                $dataVideo = $this->__uploadVideo($fi);
                $imgThumbnailVideo = $this->__uploadFotoThumb($di['img_thumbnail']);

                $di["url"] = $dataVideo;
                $di["url_type"] = "video";
                $di["img_thumbnail"] = $imgThumbnailVideo;
                $di["teks"] = ""; 
            }
        } 

        $di["cdate"] = 'NOW()';
        $this->ceb->trans_start();
        $di['nation_code'] = $nation_code;
        $di['id'] = $this->ceb->getLastId($nation_code);
        $res = $this->ceb->set($di);
        if ($res) {
            $this->ceb->trans_commit();
            $this->status = 200;
            $this->message = 'Success';
        } else {
            $this->ceb->trans_rollback();
            $this->status = 900;
            $this->message = 'Cant add promotion data to database, please try again later';
            if (file_exists(realpath($dataVideo)) && is_file(realpath($dataVideo))) {
                unlink(realpath($dataVideo));
            }
        }
        $this->ceb->trans_end();
    ...
    public function detail($id)
    {
        ...
        $detail = $this->ceb->getById($nation_code, $id);
        if (!isset($detail->id)) {
            $this->status = 455;
        ...
        $this->status = 200;
        $this->message = 'Success';
        $data = $this->ceb->getById($nation_code, $id);
        if(isset($data->judul)){
    ...
    public function edit($id)
    {
        ...
        if (!empty($id)) {
            ...
            //get last id
            // $checkdata = $this->ceb->getByIdThumbnail($nation_code, $id);
            // by Muhammad Sofi 31 January 2022 14:14 | fix bug on edit data, image thumbnail is missing
            $checkdata = $this->ceb->getById($nation_code, $id);
            ...
            $res = $this->ceb->update($nation_code, $id, $du); // by Muhammad Sofi 25 January 2022 9:43 | fix undefined checkdata
            if ($res) {
                $this->status = 200;
    ...
    public function hapus($id)
    {
        ...
        $dataurl = $this->ceb->getById($nation_code, $id);
        if (isset($dataurl->id)) {
            $res = $this->ceb->del($nation_code, $id);
            if ($res) {
    ...
    public function change_thumbnail($id){
		...
		//get last id
	    $imgThumbnail = $this->ceb->getByIdThumbnail($pengguna->nation_code, $id);
		...
		if($mv){
			...
			$this->ceb->update($pengguna->nation_code, $imgThumbnail->id, $du);
			$this->status = 200;
			$this->message = 'Success';
		}
		$this->__json_out($data);
    ...
    public function getCustomer() {
        ...
        $ddata = $this->ceb->getCustomer($nation_code, $search, 1);
    ...
    public function getProductDetail() {
        ...
        $ddata = $this->ceb->getProductDetail($nation_code, $search, 1, $seller_id);
    ...
    public function getCommunity() {
        ...
        $ddata = $this->ceb->getCommunity($nation_code, $search, 1);
