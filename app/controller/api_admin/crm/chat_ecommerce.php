...
class Chat_Ecommerce extends JI_Controller
{
    ...
    public function index()
    {
        ...
        $dcount = $this->ecpm->countAll($nation_code, $keyword);
        $ddata = $this->ecpm->getAll($nation_code, $keyword, $page, $page_size, $sortCol, $sortDir);
        ...
        foreach ($ddata as &$dt) {
            ...
            $dt->action = '<a href="'.base_url_admin('crm/chat/detail_ecommerce/'.$dt->chat_room_id.'/'.$dt->chat_type.'/'.$dt->b_user_id_1.'/'.$dt->b_user_id_2.'/0/0').'" class="btn btn-default">View Detail</button>';

            if($dt->is_read_admin == 0){
                $dt->b_user_fnama_1 .= ' <span style= "height: 15px; width: 15px; background-color: #7abce7; border-radius: 50%; display: inline-block;"></span> ';

                $dt->b_user_fnama_2 .= ' <span style= "height: 15px; width: 15px; background-color: #7abce7; border-radius: 50%; display: inline-block;"></span> ';
            }


            $chatRoomCustomer1Detail = $this->ecpm->checkRoomChat($nation_code, 0, $dt->b_user_id_1, $dt->chat_type);

            if(isset($chatRoomCustomer1Detail->id)){

                if($chatRoomCustomer1Detail->is_read_admin == 0){
                    $dt->b_user_fnama_1 .= ' <span style= "height: 15px; width: 15px; background-color: #e67e22; border-radius: 50%; display: inline-block;"></span> ';
                }

            }

            $chatRoomCustomer2Detail = $this->ecpm->checkRoomChat($nation_code, 0, $dt->b_user_id_2, $dt->chat_type);

            if(isset($chatRoomCustomer2Detail->id)){

                if($chatRoomCustomer2Detail->is_read_admin == 0){
    ...
    //         $productCustomer2 = $this->input->post("productCustomer2");

    //         if($productCustomer1 != ''){
    //             $product_id = $productCustomer1;
    //         }else if($productCustomer2 != ''){
    //             $product_id = $productCustomer2;
    //         }else{
    //             $product_id = 0;
    //         }
    ...
    //         $orderBuyerSeller = $this->input->post("orderBuyerSeller");
    //         $orderSellerBuyer = $this->input->post("orderSellerBuyer");

    //         $orderBuyerCustomer1 = $this->input->post("orderBuyerCustomer1");
    //         $orderBuyerCustomer2 = $this->input->post("orderBuyerCustomer2");

    //         $orderSellerCustomer1 = $this->input->post("orderSellerCustomer1");
    //         $orderSellerCustomer2 = $this->input->post("orderSellerCustomer2");

    //         if($orderBuyerSeller != ''){
    //             $order_id = $orderBuyerSeller;
    //         }else if($orderSellerBuyer != ''){
    //             $order_id = $orderSellerBuyer;
    //         }else if($orderBuyerCustomer1 != ''){
    //             $order_id = $orderBuyerCustomer1;
    //         }else if($orderBuyerCustomer2 != ''){
    //             $order_id = $orderBuyerCustomer2;
    //         }else if($orderSellerCustomer1 != ''){
    //             $order_id = $orderSellerCustomer1;
    //         }else if($orderSellerCustomer2 != ''){
    //             $order_id = $orderSellerCustomer2;
    //         }else{
    //             $order_id = '0/0/0';
    //         }
