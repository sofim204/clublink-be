...
class Daily_Re_Targeting extends JI_Controller{
    ...
    parent::__construct();
        ...
        $this->lib("seme_log");
		$this->load("api_admin/c_community_event_re_targeting_model",'list_model');
	...
	private function __checkDir($periode){
		...
		if(!is_dir(SENEROOT.'media/event/daily_re_targeting/')) mkdir(SENEROOT.'media/event/daily_re_targeting/',0777);
		$str = $periode.'/01';
		$periode_y = date("Y",strtotime($str));
		$periode_m = date("m",strtotime($str));
		if(!is_dir(SENEROOT.'media/event/daily_re_targeting/'.$periode_y)) mkdir(SENEROOT.'media/event/daily_re_targeting/'.$periode_y,0777);
		if(!is_dir(SENEROOT.'media/event/daily_re_targeting/'.$periode_y.'/'.$periode_m)) mkdir(SENEROOT.'media/event/daily_re_targeting/'.$periode_y.'/'.$periode_m,0777);
		return SENEROOT.'media/event/daily_re_targeting/'.$periode_y.'/'.$periode_m;
	}
	...
	public function index(){
		...
		foreach($ddata as &$gd){
            ...
            if (isset($gd->day4)) {
                if ($gd->day4 != '' && $gd->day4 != null) {
                    $day4 = json_decode($gd->day4);
                    if ($day4 != '' && $day4 != null) {
                        $count_data = count($day4);
                        if ($count_data == 3) {
                            $gd->day4 = "<center><span><i class='fa fa-check' aria-hidden='true'></i></span><br>".date("d M Y H:i:s", strtotime($gd->cdate_day_4))."</center><div style='margin-bottom: 3px;'></div>";
                        }else{
                            $gd->day4 = "";
                        }
                    }else{
            ...
            if (isset($gd->day5)) {
				$gd->day5 = "<center><span><i class='fa fa-check' aria-hidden='true'></i></span><br>".date("d M Y H:i:s", strtotime($gd->cdate_day_5))."</center><div style='margin-bottom: 3px;'></div>";
            }
	...
	public function download_xls(){
        ...
        //header
        if ($type_xls == 'agent') {
            $objWorkSheet->setCellValue('A1', '<Sending date : '.date('Y.m.d')."> Event Re-Targeting for PULSA AGENT");
        }else{
            $objWorkSheet->setCellValue('A1', '<Sending date : '.date('Y.m.d')."> Event Re-Targeting");
        }
        ...
        if(count($ddata)>0){
            foreach($ddata as $pb){
                ...
                $objWorkSheet->setCellValue('C'.$i, '20000');
	...
	public function detail($id)
    {
            ...
            if ($data->day4 != '') {
                ...
                if ($day4 != '') {
                    ...
                    foreach($day4 as $d4) {
                        ...
                        $data_d4 = $this->list_model->getDataCommunityById($nation_code, $d4); 
                        // print_r($data_d4);die; 
                        if(isset($data_d4->title)) {
                            $data->day4 .= "<a href=".base_url('a/community/listing/detail/'.$d4)." target='_blank'>$data_d4->title</a>";    
                            $enter++;
                        }            
                    } 
                }                
        ...
        if (isset($data->day5)) {
            if ($data->day5 != '') {
                $data->day5 = "<a href=".base_url('a/ecommerce/pelanggan/detail/'.$data->day5)." target='_blank'>Detail User</a>";   
            }
        }  
	...
	public function accepted() {
        ...
        $message_id = "Selamat! Anda akan menerima pulsa sebesar Rp 20.000 ke nomor telepon yang terdaftar di akun SellOn Anda. Mohon tunggu 2x24 jam agar transaksi dapat diproses.";
        $message_inggris = "Congratulations! You have received a pulsa of Rp 20.000 to the registered phone number on your SellOn account. Please allow 2x24 hours for the transaction to be processed.";
	...
	public function rejected() {
        ...
        $message_id = "Dengan menyesal kami informasikan bahwa Anda tidak memenuhi persyaratan Acara Misi Harian karena terdeteksi data yang tidak valid. Untuk bantuan lebih lanjut, silakan menghubungi kami melalui WhatsApp di Nomor Dukungan Pelanggan: +65 8856 2024 atau melalui email di support@sellon.net.";
        $message_inggris = "We regret to inform you that you did not meet the requirements for the Daily Missions Event due to detected invalid data. For further assistance, please contact us via WhatsApp at Customer Support Number: +65 8856 2024 or via email at support@sellon.net.";        
    ...
    public function finished() {
        ...
        $message_id = "Selamat! Anda telah mendapatkan reward Misi Harian untuk pengguna lama berupa pulsa dan berhasil ditransfer ke nomor yang terdaftar di aplikasi SellOn.";
        $message_inggris = "Congratulations! You have earned the Daily Missions reward for old users, which is a credit of pulsa, and it has been successfully transferred to the registered number in the SellOn application.";        
    ...
    private function __prosesRedemptionExchange($nation_code="", $id="", $admin_name="", $user=[], $data=[], $statusRedemption="", $message_id="", $message_inggris="", $event_type="", $note=nul) {
        //update table
		$du = array();
        ...
        $extras = new stdClass();
        $extras->id = $id;
        $extras->title = html_entity_decode("Retargeting Event",ENT_QUOTES);
        if($user->language_id == 2) { 
        ...
        if ($tokens){ // jika ada fcm token => push notifitication
            ...
            if($user->language_id == 2){
                $title = "Event Pengguna Lama";
                $message = $message_id;
            } else {
                $title = "Old User Event";
            ...
            $payload->id = $id;
            $payload->title = html_entity_decode("Retargeting Event",ENT_QUOTES);
            // $payload->harga_jual = $community->harga_jual;
            // $payload->foto = base_url().$community->thumb;
            if($user->language_id == 2) {
                $payload->judul = "Event Pengguna Lama";
                //by Donny Dennison
