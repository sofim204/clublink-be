...
class Emoji extends JI_Controller{
    ...
    parent::__construct();
		...
		$this->load("api_admin/c_community_emoji_model",'emoji_model');
	...
	public function index(){
		...
		$dcount = $this->emoji_model->countAll($nation_code,$keyword);
		$ddata = $this->emoji_model->getAll($nation_code,$page,$pagesize,$sortCol,$sortDir,$keyword);
		...
		foreach($ddata as &$gd){
			...
			if(isset($gd->image_icon)){
				...
				$gd->image_icon = '<img src="'.$gd->image_icon.'" class="img-responsive" style="width: 32px;" />';
			}
	...
	public function tambah(){
		...
		//start transaction
		$this->emoji_model->trans_start();
		//get last id
		$bkm2_id = $this->emoji_model->getLastId($pengguna->nation_code);
		...
		$di['image_icon'] = '';
		//insert into db
		$res = $this->emoji_model->set($di);
		if($res){
			$this->emoji_model->trans_commit();
			$this->status = 200;
			...
			$res2 = $this->emoji_model->update($pengguna->nation_code, $bkm2_id, $dux);
			if($res2){
				$this->status = 200;
				$this->message = 'Success';
				$this->emoji_model->trans_commit();
				...
				$this->emoji_model->trans_rollback();
				//delete file
				if(file_exists( SENEROOT.$filename)) unlink(SENEROOT.$filename);
			}
		}else{
	...
	public function detail($id){
		...
		$this->status = 200;
		$this->message = 'Success';
		$data = $this->emoji_model->getById($nation_code,$id);
		if(!isset($data->id)){
			$data = new stdClass();
			$this->status = 441;
	...
	public function edit($id){
		...
		$kategori = $this->emoji_model->getById($nation_code, $id);
		...
		$res = $this->emoji_model->update($pengguna->nation_code, $id, $du);
		if($res){
			$this->status = 200;
			$this->message = 'Success';
		}else{
	...
	public function hapus($id){
		...
		$kategori = $this->emoji_model->getById($pengguna->nation_code, $id);
		if(!isset($kategori->id)){
			$this->status = 520;
		...
		$res = $this->emoji_model->del($pengguna->nation_code, $id);
		if($res){
	...
	public function change_icon($id){
		...
		if($mv){
			...
			$this->emoji_model->update($pengguna->nation_code, $kategori->id, $du);
			$this->status = 200;
			$this->message = 'Success';
		}
		$this->__json_out($data);
