...
class Wait extends Runner_Controller {
  var $nation_code = '65';
  var $apikey = 'kmz373ac';
  var $apisess = '65KMZDR';
  var $cart_products = '';
  ...
  private function __testOrderDelivered(){
    $url = 'https://sellon.thecloudalert.com/api_cron/shipment/succeed/65/'.$this->order->d_order_id.'/'.$this->order->c_produk_id.'/';
    $this->__vu("Trigger Delivered",$url);
    $raw = $this->seme_curl->get($url);
    if($raw->header->http_code == 200){
      $this->__vrp("Passed");
    }else if($raw->header->http_code == 404){
  ...
  public function index(){
    ...
    //open test
    $this->__vo("Seller: New Order -> Delivery");
