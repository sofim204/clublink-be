...
class Full_Test_Rejected extends Runner_Controller
{
    public $nation_code = '65';
    public $apikey = 'kmz373ac';
    public $apisess = '65KMZDR';
    public $cart_products = '';
    public $products = '';
    public $product = '';
    public $mu = '';
    public $order = '';
    public $is_not_pass = 0;
    ...
    private function __testProductActive()
    {
        ...
        if ($raw->header->http_code == 200) {
            ...
            if (isset($result->memory)) {
                $this->mu = $result->memory;
            }
            $this->__vd($raw->body);
        } elseif ($raw->header->http_code == 404) {
            $this->__vrr("Not found");
        } elseif ($raw->header->http_code == 500) {
                ...
                if ($result->status == 200) {
                    if (isset($result->data->cart)) {
                        if (isset($result->data->cart->sellers)) {
                            if (count($result->data->cart->sellers)>0) {
                                $this->__vrp("Passed");
                                ...
                                $this->__vrr("Passed with empty result");
                            }
                        } else {
    ...
    private function __testOrderUnconfirmed()
    {
        $url = $this->url.'seller/order/new'.$this->url_aft;
        $this->__vu("Order List: Unconfirmed Order", $url);
        $raw = $this->seme_curl->get($url);
        if ($raw->header->http_code == 200) {
            $result = json_decode($raw->body);
            if ($result->status == 200) {
    ...
    private function __testOrderDetailSeller()
    {
        $url = $this->url.'seller/order/detail'.$this->url_aft;
        $this->__vu("Order Detail Seller", $url);
    ...
    private function __testOrderDetailBuyer()
    {
        $url = $this->url.'buyer/order/detail/'.$this->order->d_order_id.'/'.$this->order->d_order_detail_id.$this->url_aft;
        $this->__vu('Order Detail Buyer: '.$this->order->d_order_id.'/'.$this->order->d_order_detail_id, $url);
    ...
    private function __testOrderDetailBuyer2($d_order_id,$d_order_detail_id="")
    {
        $d_order_detail_id = (int) $d_order_detail_id;
        if($d_order_detail_id>0){
          $url = $this->url.'buyer/order/detail/'.$d_order_id.'/'.$d_order_detail_id.$this->url_aft;
          $this->__vu('Order Detail Buyer: '.$d_order_id.'/'.$d_order_detail_id, $url);
        }else{
          $url = $this->url.'buyer/order/detail/'.$d_order_id.$this->url_aft;
          $this->__vu('Order Detail Buyer: '.$d_order_id, $url);
        }

        $raw = $this->seme_curl->post($url, array("d_order_id"=>$d_order_id,"c_produk_id"=>$d_order_detail_id));
        if ($raw->header->http_code == 200) {
            $this->__vdf($raw->body);
        } elseif ($raw->header->http_code == 404) {
            $this->__vrr("Not found");
    ...
    private function __testDeliveryRejected($d_order_id, $d_order_detail_id, $d_order_detail_item_id)
    {
        $url = $this->url.'buyer/delivery/item_rejected'.$this->url_aft;
        $this->__vu("Item Rejected for CPID: ".$d_order_detail_item_id, $url);
    ...
    private function __testComplainCreate($d_order_id, $d_order_detail_id, $d_order_detail_item_id)
    {
        $url = $this->url.'buyer/complain/create'.$this->url_aft;
        $this->__vu("Create Complain for: ".$d_order_id."/".$d_order_detail_id."/".$d_order_detail_item_id, $url);
        $raw = $this->seme_curl->post($url, array("d_order_id"=>$d_order_id,"d_order_detail_id"=>$d_order_detail_id,"d_order_detail_item_id"=>$d_order_detail_item_id,'alasan'=>'test Complain Create from Seme Runner'));
        if ($raw->header->http_code == 200) {
            $result = json_decode($raw->body);
            if ($result->status == 200) {
                $this->__vrp("Passed");
    ...
    public function index()
    {
        ...
        //$this->__baseUrl("https://cms-sgmaster.sellon.net/api_mobile/");
        $this->__resetURL();
        ...
        foreach ($this->order->sellers as $seller) {
            $this->__testCheckoutShipmentRate($seller->c_produk_id, 'QXpress', 'Next Day');
        }
        ...
        $this->__setApiSess($this->apisess);
        $this->__testOrderUnconfirmed();
        if (empty($this->is_not_pass)) {
            $this->__testOrderListProcess();
        }
        $this->__testOrderDetailSeller();
        $this->__testOrderConfirmed();
        ...
        $this->__testOrderDetailBuyer();
        foreach ($this->products as $p) {
            $this->__testOrderDetailBuyer2($p->d_order_id,$p->d_order_detail_id);
            $this->__testDeliveryRejected($p->d_order_id, $p->d_order_detail_id, $p->d_order_detail_item_id);
            $this->__testComplainCreate($p->d_order_id, $p->d_order_detail_id, $p->d_order_detail_item_id);
        }
        $this->__testOrderDetailBuyer2($p->d_order_id,$p->d_order_detail_id);
        $this->__testOrderDetailBuyer2($p->d_order_id,$p->d_order_detail_id);
