...
class Delivered extends Runner_Controller {
  var $nation_code = '65';
  var $apikey = 'kmz373ac';
  var $apisess = '65KMZDR';
  var $cart_products = '';
  ...
  private function __testOrderDelivery(){
    ...
    $this->__vu("Order List: Delivered",$url);
    $raw = $this->seme_curl->post($url,array("d_order_id"=>$this->order->d_order_id,"c_produk_id"=>$this->order->c_produk_id));
    if($raw->header->http_code == 200){
      $result = json_decode($raw->body);
      if($result->status == 200){
  ...
  public function index(){
    ...
    foreach($this->order->sellers as $seller){
      $this->__testCheckoutShipmentRate($seller->c_produk_id,'Gogovan','Same Day');
    }
    ...
    if(empty($this->is_not_pass)) $this->__testOrderListProcess();
    ...
    $this->apisess = '65KMZDR'; //change to buyer session
    $this->__setApiSess($this->apisess);
