...
class OrderList extends Runner_Controller {
  ...
  private function __testOrderListWaitingForPayment(){
  ...
  private function __testOrderListPending(){
  ...
  private function __testOrderListWaitingForConfirmation(){
    $url = $this->url.'buyer/order/confirmation'.$this->url_aft;
    $this->__vu("Order Waiting For Seller Confirmation",$url);
    $raw = $this->seme_curl->get($url);
    if($raw->header->http_code == 200){
      $result = json_decode($raw->body);
      if(isset($result->memory)) $memory = $result->memory;
  ...
  public function index(){
    ...
    //open test
    $this->__vo("Buyer: Order List");

    //start call test procedure
    $this->__testOrderListWaitingForPayment();
    $this->__testOrderListPending();
    $this->__testOrderListWaitingForConfirmation();
