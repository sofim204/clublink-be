...
class Rating extends JI_Controller{
  ...
  public function index(){
    ...
    if(empty($nation_code)){
      ...
      $this->__json_out($data, (isset($nation_code)) ? $nation_code : "", (isset($pelanggan->language_id)) ? $pelanggan->language_id : "", "buyer_rating");
      die();
    }
    ...
    //get last rating
    $rating = $this->erm->getByOrderIdBuyerId($nation_code,$d_order_id,$d_order_detail_id,$pelanggan->id);
		$data = array();
		$data['rating'] = new stdClass();
		$data['rating']->seller = new stdClass();
    ...
    if(empty($nation_code)){
      ...
      die();
		...
		if($this->is_log) $this->seme_log->write("api_mobile", "API_Mobile/buyer/Rating::create -> BUID: ".$pelanggan->id." POST: ".json_encode($_POST));
		...
		$order = $this->order->getById($nation_code,$d_order_id,$d_order_detail_id);
		if(!isset($order->id)){
			$this->status = 7813;
		...
		$seller = $this->bu->getById($nation_code,$detail->b_user_id);

    //set rating from buyer
    $rating_from = 'buyer';

    //check rating if already exists
    $rating = $this->erm->check($nation_code,$d_order_id,$d_order_detail_id,$seller->id,$pelanggan->id);
    if(!isset($rating->d_order_id)){
			$this->erm->create($nation_code,$d_order_id,$d_order_detail_id,$seller->id,$pelanggan->id,0,0);
			$rating = $this->erm->check($nation_code,$d_order_id,$d_order_detail_id,$seller->id,$pelanggan->id);
    }
    //collect input
    ...
    $du = array();
    $du['seller_rating'] = $rating_value;  //from buyer to seller, fill seller_rating. CROSS method
    $res = $this->erm->update($nation_code,$d_order_id,$d_order_detail_id,$seller->id,$pelanggan->id,$du);
    if($res){
  		$this->status = 200;
  		$this->message = 'Success';
    }else{
