...
class RejectBuyer extends JI_Controller
{
    ...
    public function index()
    {
        ...
        $this->setTitle('Rejected Item(s) by Buyer'.$this->site_suffix_admin);

        $this->putThemeContent("ecommerce/rejectbuyer/home_modal", $data);
        $this->putThemeContent("ecommerce/rejectbuyer/home", $data);
        $this->putJsContent("ecommerce/rejectbuyer/home_bottom", $data);
    ...
    public function download_xls(){
        ...
        $ddata = $this->dodim->exportXlsRejectBuyer($nation_code, $keyword, $cdate_start, $cdate_end, $settlement_status);
        ...
        ->setCellValue('H1', 'Settlement Status')
        ;
        ...
        if(count($ddata)>0){
            foreach($ddata as $pb){
                //fill data to cell
                $objWorkSheet->setCellValue('A'.$i, $pb->order_id);
                ...
                $objWorkSheet->setCellValue('E'.$i, $pb->price);
                $objWorkSheet->setCellValue('F'.$i, $pb->qty);
                $objWorkSheet->setCellValue('G'.$i, $pb->sub_total);
                $objWorkSheet->setCellValue('H'.$i, $pb->settlement_status);
        ...
        $save_file = 'report_rejectbuyer-'.$pb->order_id;
