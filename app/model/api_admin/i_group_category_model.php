...
class I_Group_Category_Model extends SENE_Model {
	...
	public function getAll($nation_code, $page=0, $pagesize=10, $sortCol="cdate", $sortDir="", $keyword="", $type="category") {
		$this->db->flushQuery();
	...
	public function countAll($nation_code, $keyword="", $type="category") {
		$this->db->flushQuery();
	...
	public function getListCategory($nation_code, $keyword="", $is_active="1", $type) {
		$this->db->flushQuery();
		$this->db->select_as("$this->tbl_as.id", "id", 0);
		$this->db->select_as("$this->tbl_as.nama", "nama", 0);
		$this->db->from($this->tbl, $this->tbl_as);
		...
		if (mb_strlen($keyword)>0) {
			...
			$this->db->where_as("$this->tbl_as.nama",  addslashes($keyword), "OR", "%like%", 1, 1);
		}
		return $this->db->get("object", 0);
	}
}