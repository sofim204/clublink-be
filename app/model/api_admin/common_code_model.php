<?php
class Common_Code_Model extends SENE_Model{
	var $tbl = 'common_code';
	var $tbl_as = 'cc';
  ...
  private function __joinTbl4(){
    ...
    $composites[] = $this->db->composite_create("$this->tbl3_as.b_user_id_recruiter","=","$this->tbl4_as.id");
    return $composites;
  }
  ...
  public function update($nation_code,$id,$du){
    ...
    $this->db->select_as("$this->tbl3_as.language_id","language_id",0);
    $this->db->from($this->tbl,$this->tbl_as);
    ...
    $this->db->where_as("$this->tbl3_as.is_permanent_inactive",$this->db->esc(1));
    $this->db->where_as("$this->tbl3_as.fcm_token",$this->db->esc(""), "AND", "!=");
    $this->db->where_as("$this->tbl3_as.b_user_id_recruiter",$this->db->esc(0), "OR", "=", 1, 0);
    $this->db->where_as("$this->tbl4_as.is_permanent_inactive",$this->db->esc("1"), "AND", "=", 0, 1);
    return $this->db->get();
	}
