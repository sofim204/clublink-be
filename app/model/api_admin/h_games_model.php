<?php
class H_Games_Model extends SENE_Model {
	var $tbl = 'h_games';
	var $tbl_as = 'hg';
	...
	public function getAll($nation_code, $page=0, $pagesize=10, $sortCol="cdate", $sortDir="", $keyword="", $utype_in=array()) {
		...
		$this->db->select_as("$this->tbl_as.icon", "icon", 0);
		$this->db->select_as("$this->tbl_as.name", "name", 0);
        $this->db->select_as("$this->tbl_as.type_for_mobile", "type_for_mobile", 0);
        $this->db->select_as("$this->tbl_as.url_for_mobile", "url_for_mobile", 0);
        $this->db->select_as("$this->tbl_as.version_for_mobile", "version_for_mobile", 0);
		$this->db->select_as("$this->tbl_as.win_cost_ticket", "win_cost_ticket", 0);
		$this->db->select_as("$this->tbl_as.win_spt", "win_spt", 0);
		$this->db->select_as("$this->tbl_as.lose_cost_ticket", "lose_cost_ticket", 0);
		$this->db->select_as("$this->tbl_as.lose_spt", "lose_spt", 0);
		$this->db->select_as("$this->tbl_as.draw_cost_ticket", "draw_cost_ticket", 0);
		$this->db->select_as("$this->tbl_as.draw_spt", "draw_spt", 0);
		$this->db->select_as("$this->tbl_as.cdate", "cdate", 0);
		$this->db->select_as("$this->tbl_as.is_active", "is_active", 0);
		$this->db->select_as("$this->tbl_as.is_active_ios", "is_active_ios", 0);
		$this->db->from($this->tbl, $this->tbl_as);
