<?php
class B_Reported_User_Model extends JI_Model {
	var $tbl = 'b_user_report';
	var $tbl_as = 'bur';
	var $tbl2 = 'b_user';
	var $tbl2_as = 'bu';
	var $tbl3 = 'b_user';
	var $tbl3_as = 'bu_reported_user';
	...
	private function __joinTbl3() {
        ...
        $cps[] = $this->db->composite_create("$this->tbl_as.b_user_id_reported", "=", "$this->tbl3_as.id");
        return $cps;
    }
	...
	public function getAllReportedUser($page=0, $pagesize=10, $sortCol="sku", $sortDir="ASC", $keyword="", $from_date="", $to_date="", $path="") {
		$this->db->flushQuery();
        ...
        $this->db->select_as($this->__decrypt("$this->tbl3_as.fnama"), "reported_user_name");
        $this->db->select_as($this->__decrypt("$this->tbl3_as.email"), "reported_user_email");
		$this->db->select_as("$this->tbl_as.deskripsi", "deskripsi", 0);
		$this->db->select_as("$this->tbl_as.cdate", "reported_date", 0);
		$this->db->from($this->tbl, $this->tbl_as);
	...
	public function countAllReportedUser($keyword="", $from_date="", $to_date="", $path="") {
		$this->db->flushQuery();
