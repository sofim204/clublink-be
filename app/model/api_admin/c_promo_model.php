<?php
class C_Promo_Model extends SENE_Model {
	var $tbl = 'c_promo';
	var $tbl_as = 'cp';
	var $tbl2 = 'c_produk';
	...
	public function getAll($nation_code, $page=0, $pagesize=10, $sortCol="kode", $sortDir="ASC", $keyword="", $is_active="") {
		...
		// $this->db->select_as('id, priority, judul, gambar, utype, edate, is_active, top_bar',0);
		// $this->db->select_as('COALESCE(edate,"-")','edate',0);
		...
		$this->db->select_as("$this->tbl_as.gambar", "gambar", 0);
		$this->db->select_as("$this->tbl_as.gambar_sponsored", "gambar_sponsored", 0);
		$this->db->select_as("$this->tbl_as.type_sponsored", "type_sponsored", 0);
	...
	public function getById($nation_code, $id){
		$this->db->select_as("$this->tbl_as.*, COALESCE(edate,CURRENT_DATE())","edate");
		$this->db->from($this->tbl,$this->tbl_as);
		$this->db->where("nation_code",$nation_code);
		$this->db->where("id",$id);
	...
	public function getProductDetail($nation_code, $keyword="", $is_active="", $seller_id) {
		...
		$this->db->where_as("$this->tbl2_as.b_user_id", $this->db->esc($seller_id), "AND", "=", 0, 0);
