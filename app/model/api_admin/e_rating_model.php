...
class E_Rating_model extends SENE_Model{
  ...
  public function update($nation_code,$d_order_id,$b_user_id_seller,$b_user_id_buyer,$du){
	...
	public function create($nation_code,$d_order_id,$b_user_id_seller,$b_user_id_buyer,$seller_rating=0,$buyer_rating=0){
		$this->db->from($this->tbl,$this->tbl_as);
		$this->db->where("nation_code",$nation_code);
		$this->db->where("d_order_id",$d_order_id);
		$this->db->where("b_user_id_seller",$b_user_id_seller);
		...
		if(isset($d->d_order_id)){
			...
			return $this->db->update($nation_code,$d_order_id,$b_user_id_seller,$b_user_id_buyer,$du);
		}else{
			$di = array();
	...
	public function check($nation_code,$d_order_id,$b_user_id_seller,$b_user_id_buyer){
		$this->db->from($this->tbl,$this->tbl_as);
		$this->db->where("nation_code",$nation_code);
		$this->db->where("d_order_id",$d_order_id);
		$this->db->where("b_user_id_seller",$b_user_id_seller);
  ...
  public function getByOrderIdSellerId($nation_code,$d_order_id,$b_user_id_seller){
    $this->db->where_as("nation_code",$this->db->esc($nation_code));
    $this->db->where_as("d_order_id",$this->db->esc($d_order_id));
