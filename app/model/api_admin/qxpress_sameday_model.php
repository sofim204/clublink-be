...
class Qxpress_Sameday_Model extends SENE_Model{
    ...
    $this->db->select('weight_max');
		$this->db->select('distance_max');
		$this->db->select('cost');
		$this->db->from($this->tbl,$this->tbl_as);
		$this->db->where("nation_code",$nation_code,"AND","=",0,0);
		if(mb_strlen($keyword)>1){
			$this->db->where("id",$keyword,"OR","%like%",1,0);
      $this->db->where("weight_max",$keyword,"OR","%like%",0,0);
			$this->db->where("distance_max",$keyword,"OR","%like%",0,0);
      $this->db->where("cost",$keyword,"OR","%like%",0,1);
    }
