...
class G_Blacklist_Model extends JI_Model {
	...
	public function getAll($page=0, $pagesize=10, $sortCol="sku", $sortDir="DESC", $keyword="", $type="") {
		$this->db->flushQuery();
		$this->db->select_as("ROW_NUMBER() OVER (ORDER BY id DESC)", "no");
		$this->db->select_as("$this->tbl_as.id", "id", 0);
		$this->db->select_as("$this->tbl_as.type", "type", 0);
		$this->db->select_as("COALESCE($this->tbl_as.text,'')", "text", 0);
		...
		if(mb_strlen($keyword)>0) {
			$this->db->where("$this->tbl_as.text", $keyword, "OR", "%like%", 1, 1);
		}
