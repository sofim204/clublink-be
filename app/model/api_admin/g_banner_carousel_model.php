<?php
class G_Banner_Carousel_Model extends SENE_Model {
	var $tbl = 'g_banner_carousel';
	var $tbl_as = 'gbc';
	...
	public function getAll($nation_code, $page=0, $pagesize=10, $sortCol="kode", $sortDir="ASC", $keyword="", $is_active="") {
		...
		$this->db->select_as("$this->tbl_as.type_language", "type_language", 0);
	...
	public function getListBanner() {
		...
		$this->db->select_as("$this->tbl_as.type_language", "type_language", 0);
		$this->db->select_as("$this->tbl_as.is_active", "is_active", 0);
		$this->db->order_by("$this->tbl_as.priority", "ASC"); // by Muhammad Sofi 21 January 2022 17:46 | add sort by priority
        return $this->db->get();
    }
}
