<?php
class C_Community_Hashtag_List_Model extends JI_Model
{
    public $tbl = 'c_community_hashtag_list';
    public $tbl_as = 'cchl';
    ...
    public function getByHashtag($nation_code, $hashtag)
    {
        $this->db->select_as("$this->tbl_as.id", "id", 0);
        ...
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        $this->db->where_as("LOWER(REPLACE($this->tbl_as.name,'#',''))", $this->db->esc(strtolower(str_replace("#", "", $hashtag))));
        $this->db->where_as("$this->tbl_as.is_active", $this->db->esc(1));
        return $this->db->get_first('', 0);
    }
