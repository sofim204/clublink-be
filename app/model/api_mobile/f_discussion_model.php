<?php
class F_Discussion_Model extends JI_Model
{
    ...
    public function updateByparentId($nation_code, $parent_id, $du)
    {
        if (!is_array($du)) {
            return 0;
        }
        $this->db->where('nation_code', $nation_code);
        $this->db->where('parent_f_discussion_id', $parent_id);
        return $this->db->update($this->tbl, $du, 0);
    }
     ...
     */
    public function countAll($nation_code, $parent_discussion_id = 0, $product_id = 0)
    {
        $this->db->select_as("COUNT(DISTINCT $this->tbl_as.id)", "total", 0);
        $this->db->from($this->tbl, $this->tbl_as);
        $this->db->join_composite($this->tbl2, $this->tbl2_as, $this->__joinTbl2(), 'left');
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        $this->db->where_as("$this->tbl_as.parent_f_discussion_id", $this->db->esc($parent_discussion_id));
        $this->db->where_as("$this->tbl_as.product_id", $this->db->esc($product_id));
     ...
     */
    public function getAll($nation_code, $page=1, $page_size=10, $sort_col="cdate", $sort_direction="ASC", $parent_discussion_id=0, $product_id = 0)
    {
    ...
    public function getbyDiscussionID($nation_code, $discussion_id=0, $product_id = 0)
    {
        $this->db->select_as("$this->tbl_as.*, $this->tbl_as.id", "discussion_id", 0);
        $this->db->select_as('COALESCE('.$this->__decrypt("$this->tbl2_as.fnama").',"")', "b_user_nama", 0);
        $this->db->select_as("$this->tbl2_as.is_admin", "is_admin", 0);
    ...
    public function getLastIdNotif($nation_code, $produkid, $userid){
        $this->db->select_as("COALESCE(MAX($this->tbl_as.id),0)", "last_id", 0);
        $this->db->from($this->tbl, $this->tbl_as);
        $this->db->where("nation_code",$nation_code);
        $this->db->where("product_id",$produkid);
        $this->db->where("b_user_id",$userid);
        $d = $this->db->get_first('',0);
        if(isset($d->last_id)) return $d->last_id;
        return 0;
    }

    public function getLastId($nation_code, $parentId, $productId, $sellerid){
        $this->db->select_as("$this->tbl_as.*, $this->tbl_as.id", "discussion_id", 0);
        $this->db->select_as("$this->tbl_as.b_user_id", "b_user_id", 0);
        $this->db->select_as("$this->tbl_as.user_type", "user_type", 0);
        $this->db->select_as("$this->tbl_as.parent_f_discussion_id", "parent_f_id", 0);
        $this->db->from($this->tbl, $this->tbl_as);
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        $this->db->where_as("$this->tbl_as.product_id", $this->db->esc($productId));
        $this->db->where_as("$this->tbl_as.b_user_id", $this->db->esc($sellerid), "AND", "!=", 0, 0);
        $this->db->where_as("$this->tbl_as.parent_f_discussion_id", $this->db->esc($parentId), "OR", "=", 1, 0);
        $this->db->where_as("$this->tbl_as.id", $this->db->esc($parentId), "OR", "=", 0, 1);
        $this->db->group_by("$this->tbl_as.b_user_id");
        return $this->db->get('', 0);
    }

    // public function getDataSeller($nation_code, $dataID){
    //     $parent =0;
    //     $this->db->select_as("$this->tbl_as.*, $this->tbl_as.id", "discussion_id", 0);
    ...
    //     $this->db->join_composite($this->tbl2, $this->tbl2_as, $this->__joinTbl2(), 'left');
    //     $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
    //     $this->db->where_as("$this->tbl_as.id", $this->db->esc($dataID));
    //     $this->db->where_as("$this->tbl_as.parent_f_discussion_id", $this->db->esc($parent));
    //     $this->db->where_as("$this->tbl2_as.is_active", $this->db->esc(1));
    //     return $this->db->get_first();
    // }
/*    public function getLastId($nation_code, $getLast){
        $this->db->select_as("$this->tbl_as.*, $this->tbl_as.id", "discussion_id", 0);
        $this->db->select_as("$this->tbl_as.b_user_id", "b_user_id", 0);
        $this->db->select_as("$this->tbl_as.text", "text", 0);
        $this->db->select_as('COALESCE('.$this->__decrypt("$this->tbl2_as.fnama").',"")', "b_user_nama", 0);
        ...
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        $this->db->where_as("$this->tbl_as.product_id", $this->db->esc($productid));
        $this->db->where_as("$this->tbl_as.id", $this->db->esc($getLast), "OR", "=", 0, 0);
        $this->db->where_as("$this->tbl_as.parent_f_discussion_id", $this->db->esc($getLast));
        return $this->db->get('object', 0);
    }
*/
    ...
    public function countAllProductIDUserID($nation_code, $b_user_id = 0, $product_id = 0)
    {
        $this->db->select_as("COUNT($this->tbl_as.id)", "total", 0);
        $this->db->from($this->tbl, $this->tbl_as);
        $this->db->join_composite($this->tbl2, $this->tbl2_as, $this->__joinTbl2(), 'left');
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        $this->db->where_as("$this->tbl_as.parent_f_discussion_id", $this->db->esc(0));
        $this->db->where_as("$this->tbl_as.b_user_id", $this->db->esc($b_user_id));
        $this->db->where_as("$this->tbl_as.product_id", $this->db->esc($product_id));
        $this->db->where_as("$this->tbl_as.is_active", $this->db->esc('1'));
