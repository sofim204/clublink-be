<?php
class C_Community_Hashtag_History_For_Search_Model extends JI_Model
{
    public $tbl = 'c_community_hashtag_history_for_search';
    public $tbl_as = 'cchhfs';
    ...
    public function inactivePrevious($nation_code, $c_community_id, $du)
    {
        if (!is_array($du)) {
            return 0;
        }
    ...
    // public function del($nation_code, $c_community_id)
    // {
    //     $this->db->where("nation_code", $nation_code);
    //     $this->db->where('c_community_id', $c_community_id);
    ...
    // public function getByUseridHashtag($nation_code, $b_user_id, $hashtag, $date="")
    // {
    //     // $this->db->select_as("$this->tbl_as.id", "id", 0);
    ...
    //     $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
    //     $this->db->where_as("$this->tbl_as.b_user_id", $this->db->esc($b_user_id));
    //     $this->db->where_as("LOWER(REPLACE($this->tbl_as.hashtag,'#',''))", $this->db->esc(strtolower(str_replace("#", "", $hashtag))));
    //     if($date != ""){
