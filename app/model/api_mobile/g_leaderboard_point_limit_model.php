<?php
class G_Leaderboard_Point_Limit_Model extends JI_Model
{
    public $tbl = 'g_leaderboard_point_limit';
    public $tbl_as = 'glpt';
    public $tbl2 = 'b_user';
    public $tbl2_as = 'bu';
    ...
    public function getLastId($nation_code, $cdate)
    {
        // $this->db->select_as("COALESCE(MAX($this->tbl_as.id),0)+1", "last_id", 0);
        // $this->db->from($this->tbl, $this->tbl_as);
        // $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        // $this->db->where_as("$this->tbl_as.cdate", $this->db->esc($cdate));
        // $d = $this->db->get_first('', 0);
        // if (isset($d->last_id)) {
        //     return (int) $d->last_id;
        // }
        // return 0;
        $sql ="SELECT COALESCE(MAX(id),0)+1 AS id FROM `".$this->tbl."` WHERE id >= (SELECT COALESCE(MAX(id),0) FROM `".$this->tbl."` WHERE nation_code = '".$nation_code."' AND cdate = '".$cdate."') AND nation_code = '".$nation_code."' AND cdate = '".$cdate."' FOR UPDATE;";
    ...
    public function updateTotal($nation_code, $cdate, $b_user_id, $code, $parameter, $operator, $total)
    {
        return $this->db->exec("UPDATE `$this->tbl` SET $parameter = IF('$operator' = '+', $parameter $operator $total, IF($parameter <= 0,0,$parameter $operator $total))
            WHERE nation_code = '$nation_code' AND cdate = '$cdate' AND b_user_id = '$b_user_id' AND code = '$code';");
    }
    ...
    public function del($nation_code, $cdate)
    {
        $this->db->where_as('nation_code', $this->db->esc($nation_code));
        $this->db->where_as("cdate", $this->db->esc($cdate),"AND","<=");
...
//     }
    ...
    public function getByUserId($nation_code, $cdate, $b_user_id, $code)
    {
        $this->db->select_as("COALESCE($this->tbl_as.limit_plus,0)", "limit_plus", 0);
        $this->db->select_as("COALESCE($this->tbl_as.limit_minus,0)", "limit_minus", 0);
