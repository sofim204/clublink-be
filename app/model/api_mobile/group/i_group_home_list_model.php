...
{
    public $tbl = 'i_group_home_list';
    public $tbl_as = 'ighl';
    // public $tbl2 = 'i_group_category';
    // public $tbl2_as = 'igc';
    ...
    //     $composites[] = $this->db->composite_create("$this->tbl_as.i_group_category_id", "=", "$this->tbl2_as.id");
    //     return $composites;
    // }
    ...
    public function getAll($nation_code, $page=1, $page_size=10, $sort_col="prioritas", $sort_direction="ASC", $language_id)
    {
        ...
        if($language_id == 2){
            $this->db->order_by("$this->tbl_as.prioritas_indonesia", "ASC");
        }else{
    ...
    public function getByPrioritas($nation_code, $prioritas, $language_id)
    {
        $this->db->select_as("$this->tbl_as.id", "id", 0);
        $this->db->select_as("$this->tbl_as.type", "type", 0);
        $this->db->select_as("$this->tbl_as.i_group_sub_category_id", "i_group_sub_category_id", 0);
        ...
        $this->db->where_as("$this->tbl_as.nation_code", $this->db->esc($nation_code));
        if($language_id == 2){
            $this->db->where_as("$this->tbl_as.prioritas_indonesia", $this->db->esc($prioritas));
        }else{
