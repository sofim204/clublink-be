<?php
class D_Order_Proses_Model extends JI_Model{
  ...
  public function getByOrderIdProdukId($nation_code,$d_order_id,$c_produk_id){
	...
	public function checkByInitiatorAndNama($nation_code,$d_order_id,$c_produk_id,$initiator,$nama){
    $this->db->from($this->tbl, $this->tbl_as);
    $this->db->where("nation_code",$nation_code);
    $this->db->where("d_order_id",$d_order_id);
    $this->db->where("c_produk_id",$c_produk_id);
    $this->db->where("initiator",$initiator);
